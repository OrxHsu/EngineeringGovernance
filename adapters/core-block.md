...
Before mutating work, read `.delivery/policy.yaml` and its declared project extensions, then run `{{SOP_COMMAND}} check <absolute-project-path>`. Derive the task risk and frozen artifact requirements through `{{SOP_COMMAND}} task start --project <absolute-project-path> --input <absolute-input-path>`.
