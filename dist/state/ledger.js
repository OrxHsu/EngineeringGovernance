...
function contractReadinessErrors(input) {
    if (input.to !== 'IN_PROGRESS' || input.contract.contractReadiness?.required !== true)
        return [];
...
function safeTaskId(value, label) {
    if (!/^[a-z0-9][a-z0-9._-]*$/.test(value))
        throw new Error(`${label}_INVALID`);
}
export function canonicalTaskPath(projectRoot, taskId, name) {
...
export function readTaskLedger(input) {
    ...
    if (lines.length === 0)
        return { valid: false, errors: ['TASK_LEDGER_EMPTY'], events: [], ledgerPath };
    const events = [];
    for (const [index, line] of lines.entries()) {
        ...
        if (canonicalDigest(unsigned) !== eventDigest)
            errors.push(`TASK_EVENT_DIGEST_INVALID:${index + 1}`);
        if (event.sequence !== index + 1)
            errors.push(`TASK_EVENT_SEQUENCE_INVALID:${index + 1}`);
...
export function ledgerContentDigest(path) {
    return sha256(readFileSync(path));
}
function loadLedgerContract(projectRoot, taskId) {
    const root = realpathSync(resolve(projectRoot));
    ...
    if (!semantic.valid)
        throw new Error(`TASK_CONTRACT_INVALID:${semantic.errors.join(',')}`);
    if (contract.taskId !== taskId)
        throw new Error('TASK_CONTRACT_ID_MISMATCH');
    return { root, path, raw, contract };
}
function artifactReference(root, input) {
    if (input.kind.trim().length === 0)
        throw new Error('TASK_TRANSITION_ARTIFACT_KIND_REQUIRED');
...
export function planTaskTransition(input) {
    if (input.artifacts.length === 0)
        throw new Error('TASK_TRANSITION_ARTIFACT_REQUIRED');
    const loaded = loadLedgerContract(input.projectRoot, input.taskId);
...
function planErrors(plan, approvedDigest) {
    ...
    if (relative(loaded.root, loaded.path) !== plan.contract.path
    ...
    const previous = ledger.events.at(-1);
    if (ledgerContentDigest(ledger.ledgerPath) !== plan.ledger.beforeSha256
    ...
    if (canonicalDigest(unsignedEvent) !== eventDigest
...
export function applyTaskTransition(plan, approvedDigest) {
