...
import { inspectCleanTask } from './clean-task.js';
...
const order = ['GOOD_STANDING', 'WARNING', 'WATCH', 'PROBATION', 'SUSPENDED'];
export function recoveryStepForStanding(standing) {
    return standing === 'GOOD_STANDING' ? null : steps[standing];
}
export function generateRecoveryPlan(projectRoot, actorId) {
...
export function validateRecoveryStep(projectRoot, actorId, taskId, options = {}) {
    const status = deriveAccountabilityStatus(projectRoot, actorId);
    const cleanTask = inspectCleanTask(projectRoot, taskId);
    const errors = [...cleanTask.errors];
    if (!cleanTask.isClean || cleanTask.credit >= 0)
        errors.push('ACCOUNTABILITY_RECOVERY_TASK_NOT_CLEAN');
    ...
    const newStanding = order.find((candidate) => {
        if (candidate === 'GOOD_STANDING')
            return newScore < 3;
        if (candidate === 'WARNING')
            return newScore >= 3 && newScore < 5;
        if (candidate === 'WATCH')
            return newScore >= 5 && newScore < 8;
        if (candidate === 'PROBATION')
            return newScore >= 8 && newScore < 12;
        return newScore >= 12;
    }) ?? 'SUSPENDED';
    return {
