...
function safeTaskDirectory(projectRootInput, taskId) {
    if (!/^[a-z0-9][a-z0-9._-]*$/u.test(taskId))
        throw new Error('CLEAN_TASK_ID_INVALID');
...
export function inspectCleanTask(projectRoot, taskId) {
    const errors = [];
    let directory = '';
    let contract;
    let contractRaw;
    try {
        ...
        if (!validation.valid || contract.taskId !== taskId)
            errors.push('CLEAN_TASK_CONTRACT_INVALID');
    }
    catch (error) {
    ...
    if (contract !== undefined && contractRaw !== undefined) {
        ...
        if (!ledger.valid)
            errors.push(...ledger.errors.map((error) => `CLEAN_TASK_LEDGER_INVALID:${error}`));
        if (!closed)
            errors.push(`CLEAN_TASK_NOT_CLOSED:${ledger.currentState ?? 'UNKNOWN'}`);
        if (!noRepairRequired)
            errors.push('CLEAN_TASK_REPAIR_OR_RETRY_PRESENT');
    }
    ...
    try {
        ...
        if (!contractReviewAccepted)
            errors.push(...result.errors.map((error) => `CLEAN_TASK_CONTRACT_REVIEW_INVALID:${error}`));
    }
    catch (error) {
    ...
    try {
        ...
        if (!implementationReviewAccepted)
            errors.push(...result.errors.map((error) => `CLEAN_TASK_IMPLEMENTATION_REVIEW_INVALID:${error}`));
        if (!noImplementationFindings)
            errors.push('CLEAN_TASK_IMPLEMENTATION_FINDINGS_PRESENT');
    }
    catch (error) {
    ...
    try {
        ...
        if (!allAcceptanceFirstPass)
            errors.push('CLEAN_TASK_ACCEPTANCE_NOT_FIRST_PASS');
        if (!evidenceIntegrity)
            errors.push('CLEAN_TASK_EVIDENCE_INTEGRITY_INVALID');
        if (!authorizationIntegrity)
            errors.push('CLEAN_TASK_AUTHORIZATION_INTEGRITY_INVALID');
    }
    catch (error) {
    ...
    if (closed && Object.values(checks).every(Boolean) && risk === 'R0')
        errors.push('CLEAN_TASK_R0_NOT_ELIGIBLE');
    return {
        taskId,
        risk,
        ...checks,
