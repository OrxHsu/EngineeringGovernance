...
import { r3MandatoryDimensionsRule, scopeAcceptanceCoverageRule, sourceTestPairingRule, } from './preflight-rules.js';
...
function validatedSourceEvents(projectRoot) {
    ...
    let decoded;
    try {
        decoded = readFileSync(path, 'utf8').split('\n').filter(Boolean).map((line) => JSON.parse(line));
    }
    catch {
    ...
    for (const [index, value] of decoded.entries()) {
        if (!validateDocument('accountability-event', value).valid)
            throw new Error('ACCOUNTABILITY_PERMANENT_GATE_EVENT_SOURCE_INVALID');
        const event = value;
        const { eventDigest, ...unsigned } = value;
...
function validateGateDocument(document, actorId, projectRoot) {
    if (!validateDocument('permanent-gates', document).valid)
        throw new Error('ACCOUNTABILITY_PERMANENT_GATES_SCHEMA_INVALID');
    const { documentDigest, ...unsigned } = document;
    if (canonicalDigest(unsigned) !== documentDigest)
        throw new Error('ACCOUNTABILITY_PERMANENT_GATES_DIGEST_INVALID');
    if (document.actorId !== normalizeActorId(actorId))
        throw new Error('ACCOUNTABILITY_PERMANENT_GATES_ACTOR_MISMATCH');
    const keys = document.gates.map((gate) => `${gate.defectClass}:${gate.gateType}:${gate.rule}`);
    if (new Set(keys).size !== keys.length)
        throw new Error('ACCOUNTABILITY_PERMANENT_GATES_DUPLICATED');
    ...
    for (const gate of document.gates) {
        ...
        if (findingIndex < 0 || remediationIndex <= findingIndex)
            throw new Error('ACCOUNTABILITY_PERMANENT_GATE_PROVENANCE_INVALID');
        let prior = null;
...
function emptyDocument(actorId) {
...
export function loadPermanentGates(projectRoot, actorId) {
    const path = gatesPath(projectRoot, actorId);
    if (!existsSync(path))
        return emptyDocument(actorId);
...
export function selectPermanentGate(defectClass) {
...
export function installPermanentGate(input) {
    ...
    if (!/^[a-z0-9][a-z0-9._-]*$/u.test(input.taskId))
        throw new Error('ACCOUNTABILITY_PERMANENT_GATE_TASK_INVALID');
    if (!/^[a-f0-9]{64}$/u.test(input.remediationEventDigest))
        throw new Error('ACCOUNTABILITY_PERMANENT_GATE_EVENT_INVALID');
    ...
    if (current.gates.some((gate) => gate.gateId === gateId || gate.defectClass === defectClass))
        return current;
    const gate = {
        gateId,
        defectClass,
...
export function recordPermanentGateTrigger(input) {
    ...
    if (index < 0)
        throw new Error('ACCOUNTABILITY_PERMANENT_GATE_NOT_FOUND');
...
function authorityCompletenessErrors(input) {
    if (!Array.isArray(input.authorityInputs) || input.authorityInputs.length === 0)
        return ['PERMANENT_GATE_AUTHORITY_INPUTS_MISSING'];
    const bindings = typeof input.designBindings === 'object' && input.designBindings !== null
        && Array.isArray(input.designBindings.authorities)
        ? input.designBindings.authorities
        : [];
    return input.authorityInputs.length > bindings.length ? ['PERMANENT_GATE_AUTHORITY_BINDINGS_INCOMPLETE'] : [];
}
function ruleErrors(rule, input, risk) {
    if (rule === 'source-test-pairing')
        return sourceTestPairingRule(input).errors;
    if (rule === 'authority-completeness')
        return authorityCompletenessErrors(input);
    if (rule === 'scope-acceptance-coverage')
        return scopeAcceptanceCoverageRule(input).warnings
            .map((warning) => warning.replace('PREFLIGHT_', 'PERMANENT_GATE_'));
    if (rule === 'r3-mandatory-security')
        return r3MandatoryDimensionsRule(input, risk).errors
            .filter((error) => error.endsWith(':security'));
    return [`ACCOUNTABILITY_PERMANENT_GATE_RULE_UNKNOWN:${rule}`];
}
export function enforcePermanentGates(projectRoot, actorId, input, risk) {
    let document;
    try {
        document = loadPermanentGates(projectRoot, actorId);
    }
    catch (error) {
        return { valid: false, errors: [error instanceof Error ? error.message : 'ACCOUNTABILITY_PERMANENT_GATES_INVALID'], triggeredGateIds: [] };
    }
    const errors = [];
    const triggeredGateIds = [];
...
export function hasPermanentGates(projectRoot, actorId, defectClasses) {
