...
import { ACCOUNTABILITY_EVENTS_PATH, ACCOUNTABILITY_GENESIS_DIGEST, assertAccountabilityPolicy, normalizeDefectClass, permissionsForStanding, scoreForFinding, standingForScore, } from './policy.js';
import { policyDigestAllowedForProject, policyDigestForProject, readActorRegistry, assertAuthorizationContextTime, validateAuthorizationReference, } from './registry.js';
import { hasPermanentGates } from './permanent-gates.js';
let scopedReadCache;
export function withAccountabilityReadScope(action) {
...
function eventsPath(projectRoot) {
    ...
    if (existsSync(parent) && (lstatSync(parent).isSymbolicLink() || !lstatSync(parent).isDirectory()))
        throw new Error('ACCOUNTABILITY_EVENTS_PARENT_UNSAFE');
    return path;
}
...
function sourceSemantic(path, raw) {
    if (path.endsWith('.jsonl')) {
        try {
            ...
            throw new Error('ACCOUNTABILITY_EVENT_SOURCE_INVALID');
        }
    }
...
function exactSource(event, context, projectRoot) {
    const matches = context.bootstrap.sources.filter((candidate) => (candidate.path === event.source.artifactPath
        && candidate.rawSha256 === event.source.rawSha256
        && candidate.semanticDigest === event.source.semanticDigest));
    if (matches.length !== 1)
        throw new Error('ACCOUNTABILITY_EVENT_SOURCE_NOT_AUTHORIZED');
    ...
    if (sha256(raw) !== declared.rawSha256)
        throw new Error('ACCOUNTABILITY_EVENT_SOURCE_RAW_MISMATCH');
    if (sourceSemantic(unresolved, raw) !== declared.semanticDigest)
        throw new Error('ACCOUNTABILITY_EVENT_SOURCE_SEMANTIC_MISMATCH');
    if (!isAbsolute(declared.path)) {
        const taskMatch = /^\.delivery\/tasks\/([^/]+)\//u.exec(declared.path);
        if (taskMatch !== null && taskMatch[1] !== event.source.taskId)
            throw new Error('ACCOUNTABILITY_EVENT_SOURCE_TASK_MISMATCH');
    }
...
function exactStateFields(event, state) {
    const standing = standingForScore(state.active, state.forcedSuspended);
    if (event.lifetimePenaltyScore !== state.lifetime
...
function bootstrapActor(context, actorId) {
    const matches = context.bootstrap.actors.filter((actor) => normalizeActorId(actor.actorId) === actorId);
    if (matches.length !== 1)
        throw new Error('ACCOUNTABILITY_EVENT_ACTOR_NOT_AUTHORIZED');
    return matches[0];
}
function bootstrapSnapshot(event, state, context, sourceKind) {
    ...
    if (finding !== undefined) {
        if (normalizeActorId(String(finding.responsibleActorId)) !== event.subjectActorId
    ...
    for (const historical of context.bootstrap.findings.filter((candidate) => (candidate.responsibleActorId !== null
    ...
    for (const defectClass of actor.unresolvedDefectClasses)
        state.unresolved.add(normalizeDefectClass(defectClass));
...
function acceptedCalibrationSource(path, kind, event, state) {
    if (kind !== 'review')
        throw new Error('ACCOUNTABILITY_EVENT_TRANSITION_INVALID');
    let review;
    try {
    ...
    if (state.calibrations.includes(event.source.taskId))
        throw new Error('ACCOUNTABILITY_EVENT_TRANSITION_INVALID');
}
function validateNormalTransition(projectRoot, event, state, context, source, priorEvents) {
    if (!state.initialized && event.eventType !== 'finding_assessed')
        throw new Error('ACCOUNTABILITY_EVENT_TRANSITION_INVALID');
    if (event.eventType === 'finding_assessed') {
        const finding = context.bootstrap.findings.find((candidate) => candidate.findingId === event.source.findingId);
        const expected = finding === undefined ? undefined : scoreForFinding(finding.severity, finding.defectClass, state.offenses, finding.classification, finding.culpability);
        if (finding === undefined
            || expected === undefined
    ...
    if (event.eventType === 'calibration_recorded') {
        ...
        const ids = [event.subjectActorId, context.supervisorId, context.contractReviewerId, context.implementationReviewerId].filter((value) => value !== undefined);
    ...
    if (event.eventType === 'recognition_revoked') {
        if (state.recognition.status !== 'RELIABLE')
            throw new Error('ACCOUNTABILITY_EVENT_TRANSITION_INVALID');
        state.recognition = { status: 'NONE', grantedByEvent: state.recognition.grantedByEvent, revokedByEvent: event.eventDigest };
        exactStateFields(event, state);
        return;
    }
    const _exhaustive = event.eventType;
    void _exhaustive;
    void priorEvents;
}
function applyEvent(projectRoot, event, states, priorEvents, authorizationContexts) {
    ...
    if (event.eventType === 'standing_changed') {
        ...
        if (!state.initialized || state.calibrations.length < targetCount)
            throw new Error('ACCOUNTABILITY_EVENT_TRANSITION_INVALID');
    }
    ...
    if (state.initialized && reused)
        throw new Error('ACCOUNTABILITY_AUTHORIZATION_REPLAYED');
    if (!state.initialized)
        bootstrapSnapshot(event, state, context, source.kind);
    else
        validateNormalTransition(projectRoot, event, state, context, source, priorEvents);
    states.set(event.subjectActorId, state);
}
function parseEvent(projectRoot, decoded, index, previous) {
    if (!record(decoded) || !validateDocument('accountability-event', decoded).valid)
        throw new Error('ACCOUNTABILITY_EVENT_SCHEMA_INVALID');
    const event = decoded;
    ...
    if (canonicalDigest(unsigned) !== eventDigest)
        throw new Error('ACCOUNTABILITY_EVENT_DIGEST_INVALID');
    if (!policyDigestAllowedForProject(projectRoot, event.policyDigest))
        throw new Error('ACCOUNTABILITY_EVENT_POLICY_INVALID');
    if (previous !== undefined && Date.parse(event.occurredAt) < Date.parse(previous.occurredAt))
        throw new Error('ACCOUNTABILITY_EVENT_TIME_ORDER_INVALID');
    return { ...event, subjectActorId: normalizeActorId(event.subjectActorId) };
}
function incidentRawDigest(incident) {
    return sha256(`${JSON.stringify(incident, null, 2)}\n`);
}
function applyIncidentEvent(projectRoot, event, state) {
    ...
    const expected = scoreForFinding(incident.finding.severity, incident.finding.defectClass, state.offenses, incident.finding.classification, incident.finding.culpability);
    if (event.score.base !== expected.base
        || event.score.repeatSurcharge !== expected.repeatSurcharge
        || event.score.immediateSuspension !== expected.immediateSuspension
        || event.score.delta !== expected.delta
        || event.scoreChange?.reason !== 'finding'
...
function readAndDerive(projectRoot, appended) {
    ...
    const authorizationContexts = new Map();
    ...
    if (existsSync(path) && (lstatSync(path).isSymbolicLink() || !lstatSync(path).isFile() || realpathSync(path) !== path))
        throw new Error('ACCOUNTABILITY_EVENTS_UNSAFE');
    const decoded = existsSync(path)
        ? readFileSync(path, 'utf8').split('\n').filter(Boolean).map((line) => {
            try {
                return JSON.parse(line);
            }
            catch {
                throw new Error('ACCOUNTABILITY_EVENT_JSON_INVALID');
            }
        })
        : [];
    if (appended !== undefined)
        decoded.push(appended);
    const events = [];
    const states = new Map();
    for (const [index, value] of decoded.entries()) {
        const event = parseEvent(projectRoot, value, index, events.at(-1));
        if (!activeActors.has(event.subjectActorId))
            throw new Error('ACCOUNTABILITY_EVENT_ACTOR_UNAVAILABLE');
        if (event.incident !== undefined
    ...
    for (const actor of registry.actors.filter((candidate) => candidate.active)) {
        ...
        if (context === undefined) {
            ...
            assertAuthorizationContextTime(context, creation.occurredAt);
    ...
    if (appended === undefined)
        scopedReadCache?.set(cacheKey, result);
    return result;
}
export function readAccountabilityEvents(projectRoot) {
    return readAndDerive(projectRoot).events;
}
export function appendAccountabilityEvent(projectRoot, input) {
...
export function deriveAccountabilityStatus(projectRoot, actorOrAlias) {
...
export function eventSourceDigest(value) {
    return canonicalDigest(value);
}
export function eventRawDigest(value) {
    return sha256(value);
}
