...
import { captureRepositorySet } from './checkout-snapshot.js';
export function commandCheckId(command) {
    return `command:${canonicalDigest(command)}`;
}
function sha256(input) {
    return createHash('sha256').update(input).digest('hex');
}
function execute(command, environment = process.env) {
...
function taskOutputPath(projectRoot, segments) {
...
function captureLegacy(input) {
    if (input.runId.trim().length === 0)
        throw new Error('COMMAND_EXECUTION_RUN_ID_REQUIRED');
...
function readHardenedContract(input) {
    ...
    if (!semantic.valid)
        throw new Error(`CONTRACT_INVALID:${semantic.errors.join(',')}`);
    const { contractDigest } = contract;
    if (contract.taskId !== input.taskId)
        throw new Error('CONTRACT_TASK_ID_MISMATCH');
...
function captureHardened(input) {
    ...
    if (gate === undefined)
        throw new Error('COMMAND_EXECUTION_ACCEPTANCE_UNKNOWN');
    const repository = loaded.contract.repositories.find((candidate) => (candidate.id === gate.command.repositoryId));
    if (repository === undefined)
        throw new Error('COMMAND_EXECUTION_REPOSITORY_UNKNOWN');
    ...
    if (gate.observerPolicy.checkoutMutation === 'forbidden'
