...
export function defaultGovernanceRoot() {
...
export function renderLauncher(governanceRoot) {
    return `#!/bin/sh
set -eu
...
export function planLauncherInstall(options) {
    ...
    if (before === after && currentMode === launcherMode)
        return [];
    return [{
            path,
...
export function verifyLauncherInstall(options) {
    const path = join(resolve(options.homeDirectory), '.codex', 'bin', 'sop');
    if (!existsSync(path))
        return { valid: false, errors: ['GLOBAL_LAUNCHER_MISSING'] };
    ...
    if (readFileSync(path, 'utf8') !== expected)
        errors.push('GLOBAL_LAUNCHER_DRIFTED');
    if ((metadata.mode & 0o777) !== launcherMode)
        errors.push('GLOBAL_LAUNCHER_MODE_DRIFTED');
