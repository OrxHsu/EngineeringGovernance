...
function currentDigest(path) {
    ...
    if (lstatSync(path).isSymbolicLink())
        throw new Error(`PORTABLE_TARGET_IS_SYMLINK:${path}`);
    return sha256(readFileSync(path));
}
function plannedWrite(path, after, mode) {
    const beforeDigest = currentDigest(path);
    if (beforeDigest === sha256(after))
        return undefined;
...
export function planPortableGate(options) {
    ...
    if (basename(bundlePath) !== expectedName)
    ...
    if (!existsSync(policyPath))
        throw new Error('PROJECT_POLICY_MISSING');
    ...
    const wrapper = readFileSync(fileURLToPath(new URL('../../templates/ci/check-delivery-policy.sh', import.meta.url)));
    const writes = [
        plannedWrite(policyPath, policyContent, 0o644),
        plannedWrite(join(projectRoot, runnerRelativePath), bundle, 0o644),
        plannedWrite(join(projectRoot, '.delivery', 'bin', 'check-delivery-policy.sh'), wrapper, 0o755),
...
export function applyPortableGate(plan, reviewedDigest) {
    if (plan.digest !== reviewedDigest)
        throw new Error('PORTABLE_PLAN_DIGEST_MISMATCH');
