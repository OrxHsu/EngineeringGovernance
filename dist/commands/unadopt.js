...
function validateUnadoptionExtensions(projectRoot) {
...
export function planUnadoption(projectPath) {
    ...
    if (!schema.valid)
        throw new Error(`UNADOPTION_POLICY_INVALID:${schema.errors.join(',')}`);
    validateUnadoptionExtensions(projectRoot);
    const adapterDigests = new Map();
    ...
    if (policy.runner !== undefined) {
        ...
        if (policy.runner.version !== policy.sopVersion
        ...
        const expectedWrapper = readRunnerArchiveFile(archivePath, 'templates/ci/check-delivery-policy.sh');
        if (sha256(readFileSync(wrapperPath)) !== sha256(expectedWrapper)
...
export function summarizeUnadoptionPlan(plan) {
    return {
...
export function applyUnadoption(plan, reviewedDigest) {
