...
export function generateReviewSummary(projectRootInput, taskId) {
    ...
    const keyPoints = assisted === undefined
        ...
        : Object.entries(assisted.checklist).map(([key, item]) => (`${item.status}: ${dimensionName(key)} - ${item.observation}`));
