...
function currentDigest(path) {
    ...
    if (lstatSync(path).isSymbolicLink() || !lstatSync(path).isFile())
        throw new Error(`ACCOUNTABILITY_INITIAL_BOOTSTRAP_PATH_UNSAFE:${path}`);
...
function normalizeInput(value, projectRoot) {
    ...
    if (realpathSync(resolve(value.projectRoot)) !== projectRoot)
        throw new Error('ACCOUNTABILITY_INITIAL_BOOTSTRAP_PROJECT_INVALID');
    if (value.policyDigest !== policyDigestForProject(projectRoot))
    ...
    for (const role of ['contract-author', 'contract-reviewer', 'implementation-reviewer', 'supervisor']) {
        if (actors.filter((actor) => actor.role === role).length !== 1)
            throw new Error(`ACCOUNTABILITY_INITIAL_BOOTSTRAP_ROLE_INVALID:${role}`);
    }
    if (!actors.some((actor) => actor.role === 'implementation-owner'))
...
export function planAccountabilityBootstrap(projectRootInput, inputPathInput) {
    ...
    let decoded;
    try {
        decoded = parse(inputRaw.toString('utf8'));
    }
    catch {
        throw new Error('ACCOUNTABILITY_INITIAL_BOOTSTRAP_INPUT_INVALID');
    }
    const input = normalizeInput(decoded, projectRoot);
    ...
    if (existsSync(parent) && (lstatSync(parent).isSymbolicLink() || !lstatSync(parent).isDirectory()))
        throw new Error('ACCOUNTABILITY_INITIAL_BOOTSTRAP_PARENT_UNSAFE');
    ...
    if (existsSync(artifactPath) || existsSync(registryPath))
        throw new Error('ACCOUNTABILITY_INITIAL_BOOTSTRAP_ALREADY_EXISTS');
    if (existsSync(eventsPath) && readFileSync(eventsPath).length > 0)
        throw new Error('ACCOUNTABILITY_INITIAL_BOOTSTRAP_EVENTS_PRESENT');
    ...
    for (const [index, actor] of input.actors.entries()) {
        const unsigned = {
            ...
            actor: { id: actor.actorId, role: actor.role, trustLevel: 'local-claim' },
            occurredAt: input.issuedAt,
        };
        const event = { ...unsigned, eventDigest: canonicalDigest(unsigned) };
        if (!validateDocument('actor-registry-event', event).valid)
            throw new Error('ACCOUNTABILITY_INITIAL_BOOTSTRAP_EVENT_INVALID');
...
export function summarizeAccountabilityBootstrapPlan(plan) {
    return {
        projectRoot: plan.projectRoot,
        inputPath: plan.inputPath,
        digest: plan.digest,
...
export function applyAccountabilityBootstrap(plan, expectedDigest) {
    if (plan.digest !== expectedDigest)
        throw new Error('ACCOUNTABILITY_INITIAL_BOOTSTRAP_PLAN_MISMATCH');
    ...
    if (registry.actors.length !== plan.actorIds.length)
        throw new Error('ACCOUNTABILITY_INITIAL_BOOTSTRAP_RESULT_INVALID');
