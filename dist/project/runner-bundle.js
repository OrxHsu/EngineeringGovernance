...
function readArchiveFile(archivePath, relativePath, entries) {
    ...
    if (occurrences !== 1)
        throw new Error(`RUNNER_ARCHIVE_ENTRY_INVALID:${relativePath}:${occurrences}`);
...
export function readRunnerArchiveFile(archivePath, relativePath) {
...
export function validateRunnerBundleIdentity(input) {
    ...
    for (const path of requiredPaths) {
        ...
        if (occurrences !== 1)
            throw new Error(`RUNNER_ARCHIVE_ENTRY_INVALID:${path}:${occurrences}`);
    }
    ...
    try {
        ...
        let packageDocument;
        try {
            packageDocument = JSON.parse(read('package.json').toString('utf8'));
        }
        catch (error) {
            if (error instanceof Error && error.message.startsWith('RUNNER_ARCHIVE_'))
                throw error;
            throw new Error('RUNNER_ARCHIVE_PACKAGE_INVALID');
        }
        if (packageDocument.name !== '@xgh/engineering-governance'
