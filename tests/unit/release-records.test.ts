...
describe('release record schemas', () => {
  ...
  it('requires an archive verification receipt bound to the release record', () => {
    const receipt = {
      path: 'releases/2.1.0-beta.0/archive-verification.json', rawSha256: 'a'.repeat(64), digest: 'b'.repeat(64),
      archivePath: '/tmp/engineering-governance-2.1.0-beta.0.tgz', sha256: 'c'.repeat(64), firstBuildSha256: 'c'.repeat(64), secondBuildSha256: 'c'.repeat(64),
      candidateCommit: 'd'.repeat(40), candidateTree: 'e'.repeat(40), version: '2.1.0-beta.0', identical: true,
    }
    const record = {
      ...
      archive: { filename: 'engineering-governance-2.1.0-beta.0.tgz', path: receipt.archivePath, sha256: receipt.sha256, version: receipt.version, verificationReceipt: receipt },
      sourceIdentity: { packageVersion: '2.1.0-beta.0', versionFile: '2.1.0-beta.0' }, publicationStatus: 'local-unpublished',
      priorFinding: { path: 'releases/2.1.0-beta.0/implementation-review.yaml', rawSha256: '1'.repeat(64), digest: '2'.repeat(64) },
    }
  ...
  it('ships the canonical prior-finding record with the candidate', () => {
    const record = parse(readFileSync('releases/2.1.0-beta.0/implementation-review-75e70217.yaml', 'utf8'))
