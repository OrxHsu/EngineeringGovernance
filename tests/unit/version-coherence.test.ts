...
it('keeps package, canonical documents, and governance identity on one release version', () => {
  ...
  expect(readFileSync('README.md', 'utf8')).toContain('local `2.1.0-re` release candidate')
  ...
  expect(readFileSync('templates/project-policy.yaml', 'utf8')).toContain('sopVersion: 2.1.0-re')
  expect(readFileSync('templates/task-contract.yaml', 'utf8')).toContain('schemaVersion: 2')
  expect(readFileSync('templates/task-contract.yaml', 'utf8')).toContain('sopVersion: 2.1.0-re')
