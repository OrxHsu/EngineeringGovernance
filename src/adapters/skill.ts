...
const managedFiles = ['SKILL.md', 'agents/openai.yaml'] as const
...
function sourceMarker(sourceDirectory: string): SkillMarker {
...
function readInstalledMarker(targetDirectory: string): SkillMarker {
  const markerPath = join(targetDirectory, managedMarker)
  if (!existsSync(markerPath)) throw new Error('SKILL_TARGET_UNMANAGED')
  try {
    const marker = JSON.parse(readFileSync(markerPath, 'utf8')) as SkillMarker
    if (
...
function verifyManagedFiles(targetDirectory: string, marker: SkillMarker): void {
...
function plannedWrite(path: string, after: string): PlannedWrite | undefined {
...
function planDigest(plan: Omit<SkillInstallPlan, 'digest'>): string {
...
export function planSkillInstall(options: {
  ...
  if (markerWrite) writes.push(markerWrite)
...
export function applySkillPlan(plan: SkillInstallPlan, reviewedDigest: string): void {
  if (plan.digest !== reviewedDigest) throw new Error('SKILL_PLAN_DIGEST_MISMATCH')
