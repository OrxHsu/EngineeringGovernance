...
export interface IndependentSourceInput {
  mode: 'independent'
}

export interface SourceAssistedInput {
  mode: 'source-assisted'
  allocationId: string
  accessMode: 'inspect' | 'adapt' | 'copy-exact'
  source: {
    locator: { kind: 'git' | 'archive' | 'filesystem'; uri: string }
    pin: { algorithm: 'git-commit' | 'sha256'; digest: string }
  }
  sourceUnits: Array<{ id: string; path: string; symbols: string[] }>
  destinations: Array<{ repositoryId: string; path: string; symbols: string[] }>
  independentDestinations: Array<{ repositoryId: string; path: string; symbols: string[] }>
  releaseDecisionRequired: true
}

export type ExternalSourceTaskInput = IndependentSourceInput | SourceAssistedInput
...
function orderedBy<T>(values: T[], key: (value: T) => string): boolean {
  const keys = values.map(key)
  return JSON.stringify(keys) === JSON.stringify([...keys].sort())
}

export function validateExternalSourceTaskInput(value: unknown): ExternalSourceTaskInput {
  if (!record(value)) throw new Error('EXTERNAL_SOURCE_INPUT_INVALID')
  if (value.mode === 'independent') {
    if (!exactKeys(value, ['mode'])) throw new Error('EXTERNAL_SOURCE_INDEPENDENT_INPUT_NOT_EMPTY')
  ...
  if (!orderedBy(value.sourceUnits as SourceAssistedInput['sourceUnits'], (unit) => unit.id)) {
  ...
  for (const destination of value.destinations) {
    ...
    if (destinationIds.has(id)) throw new Error('EXTERNAL_SOURCE_DESTINATION_DUPLICATED')
    destinationIds.add(id)
  }
  if (!orderedBy(
    value.destinations as SourceAssistedInput['destinations'],
    (destination) => `${destination.repositoryId}:${destination.path}`,
  )) throw new Error('EXTERNAL_SOURCE_RECORD_ORDER_MISMATCH')
  ...
  for (const destination of value.independentDestinations) {
    ...
    const id = `${destination.repositoryId}\0${destination.path}`
    if (destinationIds.has(id)) throw new Error('EXTERNAL_SOURCE_DESTINATION_CLASSIFICATION_DUPLICATED')
    destinationIds.add(id)
  }
  if (!orderedBy(
    value.independentDestinations as SourceAssistedInput['independentDestinations'],
    (destination) => `${destination.repositoryId}:${destination.path}`,
  )) throw new Error('EXTERNAL_SOURCE_RECORD_ORDER_MISMATCH')
  if (value.releaseDecisionRequired !== true) throw new Error('EXTERNAL_SOURCE_RELEASE_DECISION_REQUIRED')
  return value as unknown as SourceAssistedInput
}

export function externalSourceMinimumRisk(input: ExternalSourceTaskInput): Risk | undefined {
...
interface SourceUseDocument {
  schemaVersion: 1
  artifactType: 'external-source-use'
  extension: { id: string; version: string; digest: string }
  taskId: string
  contractDigest: string
  allocationId: string
  sourceUses: Array<{ sourceUnitId: string; use: SourceAssistedInput['accessMode'] }>
  destinationUses: Array<{
    repositoryId: string
    path: string
    symbols: string[]
    sourceUnitIds: string[]
    use: SourceAssistedInput['accessMode']
  }>
}

interface SourceReleaseDocument {
  schemaVersion: 1
  ...
  decidedBy: { id: string; trustLevel: 'local-claim' }
...
function useAllowed(allocation: SourceAssistedInput['accessMode'], actual: SourceAssistedInput['accessMode']): boolean {
  const rank = { inspect: 0, adapt: 1, 'copy-exact': 2 }
  return rank[actual] <= rank[allocation]
}

function canonicalDestination(input: { repositoryId: string; path: string; symbols: string[] }): object {
  return {
...
export function verifyExternalSourceArtifacts(input: {
  binding: { id: string; version: string; digest: string; input: unknown }
  references: ExtensionReference[]
  projectRoot: string
  taskDirectory: string
  taskId: string
  contractDigest: string
  ledgerEvents: TaskEvent[]
  changedPaths: Array<{ repositoryId: string; path: string }>
}): { errors: string[]; result?: ExternalSourceVerificationResult } {
  const errors: string[] = []
  let taskInput: ExternalSourceTaskInput
  try {
  ...
  if (taskInput.mode === 'independent') {
    if (input.references.length > 0) errors.push('EXTERNAL_SOURCE_INDEPENDENT_HAS_ARTIFACTS')
    return {
      errors,
      ...(errors.length === 0 ? {
        result: {
  ...
  let use: SourceUseDocument
  let release: SourceReleaseDocument
  let usePath: string
  ...
  try {
    ...
    use = JSON.parse(artifact.raw.toString('utf8')) as SourceUseDocument
  } catch (error) {
  ...
  try {
    ...
    release = JSON.parse(artifact.raw.toString('utf8')) as SourceReleaseDocument
  } catch (error) {
  ...
  if (!useSchema.valid) errors.push('EXTERNAL_SOURCE_USE_SCHEMA_INVALID')
  if (!releaseSchema.valid) errors.push('EXTERNAL_SOURCE_RELEASE_SCHEMA_INVALID')
  if (!useSchema.valid || !releaseSchema.valid) return { errors: [...new Set(errors)].sort() }
  for (const document of [use, release]) {
    if (document.taskId !== input.taskId) errors.push('EXTERNAL_SOURCE_TASK_MISMATCH')
    if (document.contractDigest !== input.contractDigest) errors.push('EXTERNAL_SOURCE_CONTRACT_MISMATCH')
    if (document.allocationId !== taskInput.allocationId) errors.push('EXTERNAL_SOURCE_ALLOCATION_MISMATCH')
  ...
  if (
    !orderedBy(use.sourceUses, (entry) => entry.sourceUnitId)
  ...
  for (const destination of use.destinationUses) {
    ...
    for (const sourceUnitId of destination.sourceUnitIds) {
      ...
      if (sourceUse === undefined || !useAllowed(sourceUse, destination.use)) {
        errors.push(
          `EXTERNAL_SOURCE_USE_RELATION_MISMATCH:${destination.repositoryId}:${destination.path}:${sourceUnitId}`,
        )
      }
    }
  }
  ...
  if (release.decision !== 'approved') errors.push('EXTERNAL_SOURCE_RELEASE_BLOCKED')
  ...
  const changedPaths = [...input.changedPaths].sort((left, right) => (
    `${left.repositoryId}:${left.path}`.localeCompare(`${right.repositoryId}:${right.path}`)
  ))
  ...
  for (const [kind, path, reference] of [
    ...
    if (occurrences !== 1) errors.push(`EXTERNAL_SOURCE_LEDGER_REF_INVALID:${kind}:${occurrences}`)
  }
