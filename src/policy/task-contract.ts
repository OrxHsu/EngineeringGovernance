...
interface ContractV2 {
  ...
  selfReview?: { subjectDigest?: string }
  knownIssues?: unknown[]
...
function semanticErrors(contract: ContractV2): string[] {
  ...
  if (beta1Contract) {
    if (typeof contract.contractAuthor !== 'string' || contract.contractAuthor.length === 0) errors.push('TASK_CONTRACT_AUTHOR_REQUIRED')
    if (contract.contractPreflight === undefined || typeof contract.contractPreflight !== 'object' || contract.contractPreflight === null) {
      ...
      if (!schema.valid) errors.push(...schema.errors.map((error) => `TASK_CONTRACT_PREFLIGHT_INVALID:${error}`))
      const preflight = contract.contractPreflight as Record<string, unknown>
      const { planDigest, ...unsignedPreflight } = preflight
      if (typeof planDigest !== 'string' || canonicalDigest(unsignedPreflight) !== planDigest) errors.push('TASK_CONTRACT_PREFLIGHT_DIGEST_MISMATCH')
      if (preflight.taskId !== contract.taskId || preflight.policyDigest !== contract.policyDigest) errors.push('TASK_CONTRACT_PREFLIGHT_BINDING_MISMATCH')
    }
    if (contract.designBindings === undefined || typeof contract.designBindings !== 'object' || contract.designBindings === null) errors.push('TASK_CONTRACT_DESIGN_BINDINGS_REQUIRED')
    if (!Array.isArray(contract.predecessors)) errors.push('TASK_CONTRACT_PREDECESSORS_REQUIRED')
    ...
    if (contract.selfReview !== undefined) {
      const subjectDigest = (contract.contractPreflight as Record<string, unknown> | undefined)?.selfReviewSubjectDigest
    ...
    if (contract.openChoices !== undefined && (!Array.isArray(contract.openChoices) || contract.openChoices.length !== 0)) errors.push('TASK_CONTRACT_OPEN_CHOICES_NOT_EMPTY')
  }
