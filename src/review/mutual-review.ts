...
export type SelfReviewDimensionName = typeof SELF_REVIEW_DIMENSIONS[number]
...
export interface SelfReviewArtifact {
  schemaVersion: 1
  ...
  attemptCount: 1
  effort: 'medium'
  dimensions: SelfReviewDimension[]
  overallStatus: 'PASSED' | 'PASSED_WITH_CONCERNS' | 'TIMEOUT_SUBMITTED'
}

export interface KnownIssue {
  dimension: SelfReviewDimensionName
  observation: string
  severity: 'LOW' | 'MEDIUM'
  deferReason: string
}
...
export interface SelfReviewResponse {
  durationSeconds: number
  dimensions: SelfReviewDimension[]
  overallStatus: SelfReviewArtifact['overallStatus']
...
export interface SelfReviewRequest {
  ...
  effort: 'medium'
  timeoutSeconds: 300
  dimensions: Array<{ name: SelfReviewDimensionName; question: string }>
  prompt: string
}
...
export function mutualReviewEnabled(input: Record<string, unknown>): boolean {
  return Object.hasOwn(input, 'selfReview') || Object.hasOwn(input, 'knownIssues')
}

export function selfReviewSubject(input: Record<string, unknown>): Record<string, unknown> {
  const subject = { ...input }
...
export function selfReviewSubjectDigest(input: Record<string, unknown>): string {
  return canonicalDigest(selfReviewSubject(input))
}

export function mutualReviewErrors(input: Record<string, unknown>): string[] {
  if (!mutualReviewEnabled(input)) return []
  const errors: string[] = []
  const selfReview = input.selfReview
  const knownIssues = input.knownIssues
  if (!record(selfReview)) return ['PREFLIGHT_SELF_REVIEW_REQUIRED']
  if (!Array.isArray(knownIssues)) return ['PREFLIGHT_KNOWN_ISSUES_REQUIRED']
  ...
  if (!selfReviewSchema.valid || !knownIssuesSchema.valid) return [...new Set(errors)].sort()

  const artifact = selfReview as unknown as SelfReviewArtifact
  const issues = knownIssues as KnownIssue[]
  ...
  if (artifact.taskId !== input.taskId) errors.push('PREFLIGHT_SELF_REVIEW_TASK_MISMATCH')
  if (artifact.author !== input.contractAuthor) errors.push('PREFLIGHT_SELF_REVIEW_AUTHOR_MISMATCH')
  ...
  if (Number.isNaN(Date.parse(artifact.reviewedAt))) errors.push('PREFLIGHT_SELF_REVIEW_TIMESTAMP_INVALID')
  ...
  if (new Set(issueKeys).size !== issueKeys.length) errors.push('PREFLIGHT_KNOWN_ISSUES_DUPLICATED')
  if (artifact.overallStatus !== 'TIMEOUT_SUBMITTED') {
    ...
    for (const name of concernNames) {
      if (!issueDimensions.has(name)) errors.push(`PREFLIGHT_SELF_REVIEW_CONCERN_UNRECORDED:${name}`)
    }
    for (const issue of issues) {
      if (!concernNames.has(issue.dimension)) errors.push(`PREFLIGHT_KNOWN_ISSUE_WITHOUT_CONCERN:${issue.dimension}`)
    }
  }
  return [...new Set(errors)].sort()
}

const questions: Record<SelfReviewDimensionName, string> = {
...
function selfReviewPrompt(input: Record<string, unknown>): string {
  const acceptance = Array.isArray(input.acceptance) ? input.acceptance : []
  const acceptanceText = acceptance.map((item) => {
    if (!record(item)) return '- invalid acceptance item'
...
export function createSelfReviewRequest(
  input: Record<string, unknown>,
  inputRawSha256: string,
): SelfReviewRequest {
  if (mutualReviewEnabled(input)) throw new Error('SELF_REVIEW_SINGLE_PASS_ONLY')
...
export function finalizeSelfReview(
  input: Record<string, unknown>,
  response: SelfReviewResponse,
  reviewedAt = new Date().toISOString(),
): { selfReview: SelfReviewArtifact; knownIssues: KnownIssue[] } {
  if (mutualReviewEnabled(input)) throw new Error('SELF_REVIEW_SINGLE_PASS_ONLY')
  const subjectDigest = selfReviewSubjectDigest(input)
  const artifact: SelfReviewArtifact = {
    schemaVersion: 1,
    artifactType: 'engineering-governance-self-review-v1',
