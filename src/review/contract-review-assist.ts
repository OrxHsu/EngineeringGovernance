...
import type { SelfReviewArtifact } from './mutual-review.js'
...
export interface ContractReviewRequest {
  ...
  reviewerConstraints: {
    ...
    decisionAuthority: 'contract-review-v2-verifier'
  }
  prompt: string
}
...
function reviewPrompt(contract: Record<string, unknown>): string {
  const selfReview = record(contract.selfReview) ? contract.selfReview as unknown as SelfReviewArtifact : undefined
...
export function buildContractReviewRequest(projectRootInput: string, taskId: string): ContractReviewRequest {
