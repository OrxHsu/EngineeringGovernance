...
export interface ProjectDiscovery {
  ...
  dirtyPaths: string[]
  agentEntrypoints: AgentEntrypoint[]
}
...
function dirtyPaths(repository: string): string[] {
  const output = execFileSync(
    'git',
    ['-C', repository, 'status', '--porcelain=v1', '-z'],
...
function discoverEntrypoints(repositoryRoot: string): AgentEntrypoint[] {
...
export function discoverProject(path: string): ProjectDiscovery {
...
export function validateManagedPathOverlap(
