...
import type { TaskEvent } from '../state/ledger.js'
...
type LegacyClassification = 'beta0-beta3-history' | 'superseded-readiness-history'

interface LegacyManifestFile {
  path: string
  sha256: string
}
...
interface LegacyManifestEntry {
  ...
  contract: { rawSha256: string; digest: string }
  ledger: { rawSha256: string; finalState: TaskState; finalEventDigest: string }
  successor?: {
    taskId: string
...
export interface LegacyManifestLoad extends ValidationResult {
  manifestPath?: string
  entries: Map<string, LegacyManifestEntry>
}

export interface LegacyTaskVerification extends ValidationResult {
...
function safeProjectPath(projectRoot: string, relativePath: string): string | undefined {
  ...
  if (unresolved !== canonicalRoot && !unresolved.startsWith(`${canonicalRoot}/`)) return undefined
  const canonical = safeRegularFile(unresolved)
  return canonical === unresolved ? canonical : undefined
}

function inventory(taskRoot: string, errors: string[]): LegacyManifestFile[] {
...
function sortedUniqueFiles(files: LegacyManifestFile[]): boolean {
  return files.every((file, index) => (
    index === 0 || files[index - 1]!.path < file.path
  )) && new Set(files.map((file) => file.path)).size === files.length
}

function manifestPath(projectRoot: string, value: unknown): string | undefined {
  if (typeof value !== 'string' || !value.startsWith('.delivery/compatibility/')) return undefined
  if (value.split('/').includes('..')) return undefined
  const unresolved = resolve(projectRoot, value)
  const canonicalRoot = realpathSync(projectRoot)
  if (!unresolved.startsWith(`${canonicalRoot}/.delivery/compatibility/`)) return undefined
  const canonical = safeRegularFile(unresolved)
  return canonical === unresolved ? canonical : undefined
}

function runnerArchivePath(projectRoot: string, value: unknown): string | undefined {
  if (typeof value !== 'string' || !value.startsWith('.delivery/runtime/')) return undefined
  if (value.split('/').includes('..')) return undefined
  const unresolved = resolve(projectRoot, value)
  const canonicalRoot = realpathSync(projectRoot)
  if (!unresolved.startsWith(`${canonicalRoot}/.delivery/runtime/`)) return undefined
  const canonical = safeRegularFile(unresolved)
  return canonical === unresolved ? canonical : undefined
}

export function loadLegacyCompatibilityManifest(
  projectRootInput: string,
  policy: Record<string, unknown> | undefined,
): LegacyManifestLoad {
  const entries = new Map<string, LegacyManifestEntry>()
  ...
  if (sha256(raw) !== configuredSha) errors.push('TASK_GRAPH_LEGACY_MANIFEST_SHA_MISMATCH')
  let document: unknown
  try { document = parse(raw.toString('utf8')) } catch { document = undefined }
  const schema = validateDocument('legacy-task-compatibility', document)
  if (!schema.valid) errors.push(...schema.errors.map((error) => `TASK_GRAPH_LEGACY_MANIFEST_INVALID:${error}`))
  if (schema.valid && document !== null && typeof document === 'object' && !Array.isArray(document)) {
    const manifest = document as LegacyManifest
    ...
    if (manifest.entries.length === 0 || !manifest.entries.every((entry, index) => (
      index === 0 || manifest.entries[index - 1]!.taskId < entry.taskId
    ))) errors.push('TASK_GRAPH_LEGACY_MANIFEST_ENTRIES_NOT_SORTED_UNIQUE')
    for (const entry of manifest.entries) {
      if (entries.has(entry.taskId)) errors.push(`TASK_GRAPH_LEGACY_MANIFEST_DUPLICATE_TASK:${entry.taskId}`)
      ...
      if (entry.successor !== undefined && !entry.successor.findingIds.every((findingId, index) => (
        index === 0 || entry.successor!.findingIds[index - 1]! < findingId
      ))) errors.push(`TASK_GRAPH_LEGACY_MANIFEST_FINDINGS_NOT_SORTED_UNIQUE:${entry.taskId}`)
    }
    if (manifest.projectId !== policy?.projectId) errors.push('TASK_GRAPH_LEGACY_MANIFEST_PROJECT_MISMATCH')
  }
  return {
...
function runnerErrors(projectRoot: string, entry: LegacyManifestEntry): string[] {
  const errors: string[] = []
  if (entry.source.runner.version !== entry.source.sopVersion) errors.push('RUNNER_SOURCE_VERSION_MISMATCH')
  const path = runnerArchivePath(projectRoot, entry.source.runner.path)
  if (path === undefined) return ['RUNNER_ARCHIVE_INVALID']
  if (sha256(readFileSync(path)) !== entry.source.runner.sha256) errors.push('RUNNER_ARCHIVE_DIGEST_MISMATCH')
  try {
    const packageJson = JSON.parse(readRunnerArchiveFile(path, 'package.json').toString('utf8')) as {
      name?: unknown
      version?: unknown
    }
...
function ledgerErrors(
  ...
  if (ledgerCanonical === undefined) return ['LEDGER_MISSING_OR_UNSAFE']
  const raw = readFileSync(ledgerCanonical)
  if (sha256(raw) !== entry.ledger.rawSha256) errors.push('LEDGER_DIGEST_MISMATCH')
  const lines = raw.toString('utf8').split('\n').filter((line) => line.length > 0)
  if (lines.length === 0) return ['LEDGER_EMPTY']
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (canonicalDigest(unsigned) !== eventDigest) errors.push(`LEDGER_EVENT_DIGEST_INVALID:${index + 1}`)
    if (event.sequence !== index + 1) errors.push(`LEDGER_EVENT_SEQUENCE_INVALID:${index + 1}`)
    if (event.contractDigest !== contractDigest) errors.push(`LEDGER_EVENT_CONTRACT_MISMATCH:${index + 1}`)
    ...
    if (index === 0) {
      ...
      if (contractRef?.kind !== 'contract'
        ...
        || contractRef.sha256 !== sha256(contractRaw)) errors.push('LEDGER_INITIAL_CONTRACT_REF_MISMATCH')
    } else if (previous !== undefined) {
      if (event.previousEventDigest !== previous.eventDigest) errors.push(`LEDGER_PREVIOUS_DIGEST_MISMATCH:${index + 1}`)
...
function predecessorGraphErrors(
  projectRoot: string,
  taskId: string,
  compatibilityTaskIds: ReadonlySet<string>,
  stack: Set<string>,
  visited: Set<string>,
): string[] {
  if (compatibilityTaskIds.has(taskId)) return ['SUCCESSOR_PREDECESSOR_COMPATIBILITY_CYCLE']
  if (stack.has(taskId)) return ['SUCCESSOR_PREDECESSOR_GRAPH_CYCLE']
  if (visited.has(taskId)) return []
  const path = join(projectRoot, '.delivery', 'tasks', taskId, 'contract.yaml')
  const canonical = safeRegularFile(path)
  if (canonical === undefined) return ['SUCCESSOR_PREDECESSOR_CONTRACT_MISSING_OR_UNSAFE']
  let contract: unknown
  try { contract = parse(readFileSync(canonical, 'utf8')) as unknown } catch {
  ...
  if (Array.isArray(contract.predecessors)) {
    for (const predecessor of contract.predecessors) {
      ...
      errors.push(...predecessorGraphErrors(
        projectRoot,
        predecessor.taskId,
...
function successorErrors(
  projectRoot: string,
  entry: LegacyManifestEntry,
  compatibilityTaskIds: ReadonlySet<string>,
): string[] {
  if (entry.classification !== 'superseded-readiness-history') return []
  if (entry.successor === undefined
    || entry.successor.taskId === entry.taskId
    || compatibilityTaskIds.has(entry.successor.taskId)) return ['SUCCESSOR_BINDING_INVALID']
  const path = join(projectRoot, '.delivery', 'tasks', entry.successor.taskId, 'contract.yaml')
  const canonical = safeRegularFile(path)
  if (canonical === undefined) return ['SUCCESSOR_CONTRACT_MISSING_OR_UNSAFE']
  const raw = readFileSync(canonical)
  if (sha256(raw) !== entry.successor.contractRawSha256) return ['SUCCESSOR_CONTRACT_DIGEST_MISMATCH']
  let parsedContract: unknown
  try { parsedContract = parse(raw.toString('utf8')) as unknown } catch { return ['SUCCESSOR_CONTRACT_UNREADABLE'] }
  if (!record(parsedContract)) return ['SUCCESSOR_CONTRACT_INVALID']
  ...
  if (!schema.valid) return ['SUCCESSOR_CONTRACT_SCHEMA_INVALID']
  const predecessors = contract.predecessors
  if (!Array.isArray(predecessors)) return ['SUCCESSOR_PREDECESSOR_BINDING_MISSING']
  const matchingPredecessors = predecessors.filter((item) => (
    typeof item === 'object' && item !== null && !Array.isArray(item)
      && (item as Record<string, unknown>).taskId === entry.taskId
  ...
  try { parsedReview = parse(readFileSync(reviewCanonical, 'utf8')) as unknown } catch {
    return ['SUCCESSOR_PREDECESSOR_REVIEW_UNREADABLE']
  }
  if (!record(parsedReview)) return ['SUCCESSOR_PREDECESSOR_REVIEW_INVALID']
  ...
  for (const other of predecessors) {
    if (!record(other) || typeof other.taskId !== 'string' || other.taskId === entry.taskId) continue
    errors.push(...predecessorGraphErrors(
      projectRoot,
      other.taskId,
      compatibilityTaskIds,
      new Set([entry.successor.taskId]),
      new Set(),
    ))
  }
...
export function verifyLegacyTaskCompatibility(input: {
  ...
  contractRaw: Buffer
  entry: LegacyManifestEntry | undefined
  compatibilityTaskIds: ReadonlySet<string>
}): LegacyTaskVerification {
  if (input.entry === undefined) return { valid: false, errors: ['TASK_GRAPH_LEGACY_ENTRY_MISSING'] }
  const entry = input.entry
  const errors: string[] = []
  if (entry.taskId !== input.taskId) errors.push('TASK_ID_MISMATCH')
  if (input.contract.schemaVersion !== 2 || input.contract.taskId !== input.taskId) errors.push('CONTRACT_SCHEMA_OR_TASK_MISMATCH')
  ...
  if (sha256(input.contractRaw) !== entry.contract.rawSha256) errors.push('CONTRACT_RAW_DIGEST_MISMATCH')
  ...
  try {
    ...
    if (JSON.stringify(actualFiles) !== JSON.stringify(entry.files)) errors.push('TASK_FILE_INVENTORY_MISMATCH')
...
export function legacyManifestTaskIds(manifest: LegacyManifestLoad): string[] {
