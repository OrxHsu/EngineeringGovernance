...
interface HistoricalEvidenceManifest {
  schemaVersion: 1
  artifactType: 'engineering-governance-historical-evidence-compatibility-v1'
  projectId: string
  manifestDigest: string
  entries: HistoricalEvidenceEntry[]
}

export interface HistoricalEvidenceLoad extends ValidationResult {
  entries: Map<string, HistoricalEvidenceEntry>
}
...
function evidenceDigest(raw: Buffer, originalPath: string): string {
...
function safeFile(root: string, relativePath: string, prefix: string): string | undefined {
  if (!relativePath.startsWith(prefix) || relativePath.split('/').includes('..')) return undefined
  const path = resolve(root, relativePath)
  if (!path.startsWith(`${root}/${prefix}`)) return undefined
  try {
...
export function historicalEvidenceKey(input: Pick<HistoricalEvidenceEntry, 'path' | 'sha256' | 'digest'>): string {
  return `${input.path}\0${input.sha256}\0${input.digest}`
}

export function loadHistoricalEvidence(projectRootInput: string): HistoricalEvidenceLoad {
  const entries = new Map<string, HistoricalEvidenceEntry>()
  const root = realpathSync(resolve(projectRootInput))
  const policyPath = resolve(root, '.delivery/policy.yaml')
  if (!existsSync(policyPath)) return { valid: true, errors: [], entries }
  let policy: unknown
  try { policy = parse(readFileSync(policyPath, 'utf8')) as unknown } catch {
  ...
  if (configuredPath === undefined && configuredSha === undefined) return { valid: true, errors: [], entries }
  ...
  if (manifestPath === undefined) return { valid: false, errors: ['HISTORICAL_EVIDENCE_MANIFEST_PATH_INVALID'], entries }
  const raw = readFileSync(manifestPath)
  const errors: string[] = []
  if (sha256(raw) !== configuredSha) errors.push('HISTORICAL_EVIDENCE_MANIFEST_SHA_MISMATCH')
  let document: unknown
  try { document = parse(raw.toString('utf8')) as unknown } catch { document = undefined }
  const schema = validateDocument('historical-evidence-compatibility', document)
  if (!schema.valid) errors.push(...schema.errors.map((error) => `HISTORICAL_EVIDENCE_MANIFEST_INVALID:${error}`))
  if (schema.valid && record(document)) {
    const manifest = document as unknown as HistoricalEvidenceManifest
    const { manifestDigest, ...unsigned } = manifest
    if (canonicalDigest(unsigned) !== manifestDigest) errors.push('HISTORICAL_EVIDENCE_MANIFEST_DIGEST_MISMATCH')
    if (manifest.projectId !== policy.projectId) errors.push('HISTORICAL_EVIDENCE_PROJECT_MISMATCH')
    let previous = ''
    for (const entry of manifest.entries) {
      const key = historicalEvidenceKey(entry)
      if (key <= previous || entries.has(key)) errors.push('HISTORICAL_EVIDENCE_ENTRIES_NOT_SORTED_UNIQUE')
      previous = key
