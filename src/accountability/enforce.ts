...
interface LifecycleAuthorizationBinding {
  path: string
  rawSha256: string
...
export interface RemediationAuthorization {
  schemaVersion: 2
  artifactType: 'engineering-governance-remediation-authorization-v1'
  authorizationId: string
  requirementId: string
  taskId: string
  contract: { path: string; rawSha256: string; semanticDigest: string }
  lifecycleAuthorization: LifecycleAuthorizationBinding
  ...
  implementationReviewerId: string
  issuedAt: string
  expiresAt: string
  consumeOnce: true
  status: 'approved'
...
interface HistoricalRemediationAuthorization extends Omit<RemediationAuthorization, 'lifecycleAuthorization'> {}

interface LedgerArtifactReference {
  kind: string
  path: string
  sha256: string
}
...
interface LedgerEvent {
  ...
  actorId: string
  contractDigest: string
  artifactRefs: LedgerArtifactReference[]
  eventDigest: string
}

export interface RemediationBridgeVerification {
  applicable: boolean
  valid: boolean
  errors: string[]
  contract?: BridgeContract
  lifecycleAuthorization?: StandardAuthorization
  lifecycleAuthorizationPath?: string
  lifecycleAuthorizationRawSha256?: string
  lifecycleAuthorizationSemanticDigest?: string
  sidecar?: RemediationAuthorization
  sidecarPath?: string
  sidecarRawSha256?: string
  sidecarSemanticDigest?: string
}
...
function safeTaskId(value: string): void {
  if (!/^[a-z0-9][a-z0-9._-]*$/.test(value)) throw new Error('ACCOUNTABILITY_TASK_ID_INVALID')
}

function safeContainedFile(projectRoot: string, inputPath: string, label: string): string {
  ...
  if (relativePath.startsWith('..') || isAbsolute(relativePath)) throw new Error(`${label}_OUTSIDE_PROJECT`)
  ...
  if (path !== unresolved) throw new Error(`${label}_NONCANONICAL`)
  return path
}

function taskFile(projectRoot: string, taskId: string, ...segments: string[]): string {
  safeTaskId(taskId)
  const root = realpathSync(resolve(projectRoot))
...
function loadContract(projectRoot: string, taskId: string): {
  contract: BridgeContract
  path: string
  raw: Buffer
} {
  ...
  const contract = parse(raw.toString('utf8')) as BridgeContract
  const validation = validateHardenedTaskContract(contract)
  if (!validation.valid) throw new Error(`ACCOUNTABILITY_CONTRACT_INVALID:${validation.errors.join(',')}`)
  if (contract.taskId !== taskId) throw new Error('ACCOUNTABILITY_CONTRACT_TASK_MISMATCH')
  const { contractDigest, ...unsigned } = contract
  if (canonicalDigest(unsigned) !== contractDigest) throw new Error('ACCOUNTABILITY_CONTRACT_DIGEST_INVALID')
  return { contract, path, raw }
}

function requirements(contract: unknown): AuthorizationRequirement[] {
  if (!record(contract) || !Array.isArray(contract.authorizationRequirements)) return []
  return contract.authorizationRequirements.filter((value): value is AuthorizationRequirement => (
    record(value)
...
function authorityInputs(contract: unknown): string[] {
  if (!record(contract) || !Array.isArray(contract.authorityInputs)) return []
  return contract.authorityInputs.filter((value): value is string => typeof value === 'string')
}

export function isRemediationBridgeContract(contract: unknown): boolean {
...
function isHistoricalRemediationContract(contract: unknown): boolean {
  return requirements(contract).some((requirement) => (
    requirement.consumeOnce
    && requirement.scope.some((entry) => entry.includes('engineering-governance-remediation-authorization-v1'))
  ))
}

export function isAccountabilityContract(contract: unknown, _taskId?: string): boolean {
  if (!record(contract)) return false
  if (contract.contractAuthor !== undefined || contract.contractPreflight !== undefined || contract.designBindings !== undefined || contract.predecessors !== undefined) return true
  return isRemediationBridgeContract(contract) || isHistoricalRemediationContract(contract)
}

function authorizationWindow(scope: string[]): { issuedAt: string; expiresAt: string } | undefined {
...
function validateTimeWindow(
  issuedAt: string,
  expiresAt: string,
  expected: { issuedAt: string; expiresAt: string } | undefined,
  enforceExpiry: boolean,
): string[] {
  const errors: string[] = []
  const issued = Date.parse(issuedAt)
  const expires = Date.parse(expiresAt)
  if (!Number.isFinite(issued) || !Number.isFinite(expires) || issued >= expires) errors.push('ACCOUNTABILITY_AUTHORIZATION_TIME_RANGE_INVALID')
  ...
  if (enforceExpiry && Number.isFinite(expires) && Date.now() >= expires) errors.push('ACCOUNTABILITY_AUTHORIZATION_EXPIRED')
  if (enforceExpiry && Number.isFinite(issued) && Date.now() < issued) errors.push('ACCOUNTABILITY_AUTHORIZATION_NOT_YET_VALID')
  return errors
}

function readLedger(path: string): { events: LedgerEvent[]; errors: string[] } {
  const errors: string[] = []
  const events: LedgerEvent[] = []
  const lines = readFileSync(path, 'utf8').split('\n').filter(Boolean)
  for (const [index, line] of lines.entries()) {
    let event: LedgerEvent
    try { event = JSON.parse(line) as LedgerEvent } catch {
    ...
    if (canonicalDigest(unsigned) !== eventDigest) errors.push(`ACCOUNTABILITY_PREDECESSOR_LEDGER_DIGEST_INVALID:${index + 1}`)
    if (event.sequence !== index + 1) errors.push(`ACCOUNTABILITY_PREDECESSOR_LEDGER_SEQUENCE_INVALID:${index + 1}`)
    const previous = events.at(-1)
    if (index === 0) {
      if (event.previousEventDigest !== null || event.from !== null || event.to !== 'DEFINED') errors.push('ACCOUNTABILITY_PREDECESSOR_LEDGER_INITIAL_INVALID')
...
function acceptedReviewErrors(input: {
  ...
  try {
    ...
    review = parse(readFileSync(path, 'utf8')) as Record<string, unknown>
  } catch {
    return ['ACCOUNTABILITY_CONTRACT_REVIEW_UNREADABLE']
  }
  const schema = validateDocument('contract-review', review)
  if (!schema.valid) errors.push(...schema.errors.map((error) => `ACCOUNTABILITY_CONTRACT_REVIEW_SCHEMA_INVALID:${error}`))
...
function authoritySemanticDigest(path: string, raw: Buffer): string {
  ...
  try { return canonicalDigest(parse(raw.toString('utf8'))) } catch {
    throw new Error('ACCOUNTABILITY_PREDECESSOR_BOOTSTRAP_SOURCE_INVALID')
  }
}

function predecessorBootstrapBinding(projectRoot: string, contract: BridgeContract): {
  defectPath: string
  ...
  const bootstrap = parse(readFileSync(bootstrapPath, 'utf8')) as unknown
  ...
  const sources = bootstrap.sources.filter((source): source is Record<string, unknown> => record(source))
  ...
  if (match === null) throw new Error('ACCOUNTABILITY_PREDECESSOR_BOOTSTRAP_BINDING_INVALID')
  ...
  if (predecessorSources.length !== expected.size) throw new Error('ACCOUNTABILITY_PREDECESSOR_BOOTSTRAP_BINDING_INVALID')
  ...
  if ([...expected.keys()].some((path) => !declared.has(path))) throw new Error('ACCOUNTABILITY_PREDECESSOR_BOOTSTRAP_BINDING_INVALID')
  const defectInputs = authorityInputs(contract).filter((value) => value.endsWith('/contract-defect.yaml'))
...
function predecessorDefectErrors(projectRoot: string, contract: BridgeContract): string[] {
  const errors: string[] = []
  const root = realpathSync(resolve(projectRoot))
  let binding: ReturnType<typeof predecessorBootstrapBinding>
  try { binding = predecessorBootstrapBinding(root, contract) } catch {
    return ['ACCOUNTABILITY_PREDECESSOR_BOOTSTRAP_BINDING_INVALID']
  }
  let defectPath: string
  let defectRaw: Buffer
  let defect: Record<string, unknown>
  try {
    defectPath = safeContainedFile(root, binding.defectPath, 'ACCOUNTABILITY_PREDECESSOR_DEFECT')
    defectRaw = readFileSync(defectPath)
    defect = parse(defectRaw.toString('utf8')) as Record<string, unknown>
  } catch {
  ...
  if (
    defect.artifactType !== 'engineering-governance-contract-defect-v1'
  ...
  if (typeof defect.taskId !== 'string') return errors
  ...
  for (const authority of requiredAuthorities) {
    if (!declared.has(authority)) errors.push(`ACCOUNTABILITY_PREDECESSOR_AUTHORITY_MISSING:${authority}`)
  }

  let predecessorContractPath: string
  let predecessorContractRaw: Buffer
  let predecessorContract: Record<string, unknown>
  let predecessorReviewRaw: Buffer
  try {
    ...
    predecessorContract = parse(predecessorContractRaw.toString('utf8')) as Record<string, unknown>
  ...
  if (
    predecessorContract.taskId !== predecessorTaskId
...
export function remediationBridgeErrors(input: {
  ...
  taskId: string
  actorId: string
  role: 'contract-author' | 'implementation-owner'
  enforceExpiry?: boolean | undefined
  ledgerEvents?: LedgerEvent[]
  requireConsumption?: boolean
}): RemediationBridgeVerification {
  const errors: string[] = []
  let loaded: ReturnType<typeof loadContract>
  try { loaded = loadContract(input.projectRoot, input.taskId) } catch (error) {
    return { applicable: false, valid: false, errors: [error instanceof Error ? error.message : 'ACCOUNTABILITY_CONTRACT_UNREADABLE'] }
  }
  if (!isRemediationBridgeContract(loaded.contract)) return { applicable: false, valid: false, errors: [] }
  ...
  let lifecycleRaw: Buffer
  let lifecycle: StandardAuthorization
  let sidecarRaw: Buffer
  let sidecar: RemediationAuthorization
  try {
    ...
    lifecycle = JSON.parse(lifecycleRaw.toString('utf8')) as StandardAuthorization
    ...
    if (!schema.valid || lifecycle.artifactType !== 'sop-authorization-v2') {
      ...
      if (lifecycle.artifactType !== 'sop-authorization-v2') errors.push('ACCOUNTABILITY_LIFECYCLE_AUTHORIZATION_TYPE_INVALID')
    }
  } catch {
  ...
  try {
    ...
    sidecar = JSON.parse(sidecarRaw.toString('utf8')) as RemediationAuthorization
    ...
    if (!schema.valid || sidecar.artifactType !== 'engineering-governance-remediation-authorization-v1' || !record(sidecar.lifecycleAuthorization)) {
      ...
      if (!record(sidecar.lifecycleAuthorization)) errors.push('ACCOUNTABILITY_REMEDIATION_SIDECAR_LIFECYCLE_BINDING_MISSING')
    }
  } catch {
  ...
  if (
    lifecycle.requirementId !== requirement.id
  ...
  if (
    sidecar.taskId !== loaded.contract.taskId
  ...
  if (
    !isImplementationOwner(loaded.contract, input.actorId)
  ...
  if (input.requireConsumption === true) {
    ...
    if (lifecycleOccurrences !== 1) errors.push(`ACCOUNTABILITY_LIFECYCLE_AUTHORIZATION_CONSUMPTION_INVALID:${lifecycleOccurrences}`)
    if (sidecarOccurrences !== 1) errors.push(`ACCOUNTABILITY_REMEDIATION_SIDECAR_CONSUMPTION_INVALID:${sidecarOccurrences}`)
    if (candidateEvents.length !== 1) errors.push(`ACCOUNTABILITY_REMEDIATION_CANDIDATE_BINDING_INVALID:${candidateEvents.length}`)
  }
...
function historicalRemediationErrors(input: {
  ...
  let loaded: ReturnType<typeof loadContract>
  try { loaded = loadContract(input.projectRoot, input.taskId) } catch {
  ...
  if (values.length !== 1) return { applicable: true, errors: ['ACCOUNTABILITY_HISTORICAL_AUTHORIZATION_REQUIREMENT_SET_INVALID'] }
  const requirement = values[0]!
  let authorization: HistoricalRemediationAuthorization
  try {
    const path = taskFile(input.projectRoot, input.taskId, 'authorizations', `${requirement.id}.json`)
    authorization = JSON.parse(readFileSync(path, 'utf8')) as HistoricalRemediationAuthorization
  ...
  errors.push(...validateTimeWindow(
    authorization.issuedAt,
    authorization.expiresAt,
    authorizationWindow(requirement.scope),
    input.enforceExpiry,
  ))
  return { applicable: true, errors: [...new Set(errors)].sort() }
}

export function readRemediationAuthorization(projectRoot: string, taskId: string): RemediationAuthorization {
...
export function authorizationDigest(projectRoot: string, taskId: string): { rawSha256: string; semanticDigest: string; path: string } {
  const verification = remediationBridgeErrors({
    projectRoot,
    taskId,
    actorId: primaryImplementationOwner(loadContract(projectRoot, taskId).contract),
...
export function assertStanding(projectRoot: string, actorId: string, minimum: Standing): void {
  const status = deriveAccountabilityStatus(projectRoot, actorId)
  const rank: Record<Standing, number> = { GOOD_STANDING: 0, WARNING: 1, WATCH: 2, PROBATION: 3, SUSPENDED: 4 }
  if (rank[status.standing] > rank[minimum]) throw new Error(`ACCOUNTABILITY_STANDING_INSUFFICIENT:${status.standing}:${minimum}`)
}

export function assertActorSeparation(...actorIds: string[]): void {
  const normalized = actorIds.map(normalizeActorId)
  if (new Set(normalized).size !== normalized.length) throw new Error('ACCOUNTABILITY_ACTOR_SEPARATION_INVALID')
}
...
export function assertRemediationScope(input: {
  ...
  if (
    verification.sidecar.action !== input.action
...
export function assertNoCallerStanding(value: unknown): never {
  throw new Error(`ACCOUNTABILITY_CALLER_STANDING_IGNORED:${typeof value}`)
}

export type AccountabilityRole = 'contract-author' | 'implementation-owner' | 'contract-reviewer' | 'implementation-reviewer' | 'closer'
...
export function actorEligibilityErrors(input: {
  ...
  try { standing = deriveAccountabilityStatus(input.projectRoot, input.actorId).standing } catch { return ['ACCOUNTABILITY_ACTOR_UNAVAILABLE'] }
  if (standing === 'GOOD_STANDING') return []
  if (standing === 'WARNING') {
    if (input.risk === 'R3' || input.role === 'contract-reviewer' || input.role === 'implementation-reviewer') return ['ACCOUNTABILITY_WARNING_ROLE_FORBIDDEN']
    if (input.risk === 'R2') return ['ACCOUNTABILITY_WARNING_SUPERVISION_REQUIRED']
    return []
  }
  if (standing === 'WATCH') {
    if (input.risk === 'R3' || input.role === 'contract-reviewer' || input.role === 'implementation-reviewer') return ['ACCOUNTABILITY_WATCH_ROLE_FORBIDDEN']
  ...
  if (input.role === 'contract-author' || input.role === 'implementation-owner') {
    ...
    if (bridge.applicable) return bridge.errors
    ...
    if (historical.applicable) return historical.errors
  }
