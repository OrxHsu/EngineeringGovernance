...
import {
  ACCOUNTABILITY_GENESIS_BYTES,
  ACCOUNTABILITY_GENESIS_DIGEST,
  ACCOUNTABILITY_REGISTRY_PATH,
...
export type ActorAuthorization = AuthorizationReference

export interface AccountabilityBootstrapSource {
  id: string
  kind: 'contract' | 'review' | 'ledger' | 'report' | 'defect'
  path: string
  rawSha256: string
  semanticDigest: string
}

export interface AccountabilityBootstrapActor {
  actorId: string
  aliases: string[]
  lifetimePenaltyScore: number
  activePenaltyScore: number
  standing: 'GOOD_STANDING' | 'WARNING' | 'WATCH' | 'PROBATION' | 'SUSPENDED'
  permissions: string[]
  unresolvedDefectClasses: string[]
}

export interface AccountabilityBootstrapFinding {
  findingId: string
...
export interface AccountabilityBootstrap {
  taskId: string
  sources: AccountabilityBootstrapSource[]
  findings: AccountabilityBootstrapFinding[]
  actors: AccountabilityBootstrapActor[]
  remediationException: Record<string, unknown>
}

export interface AuthorizationContext {
  reference: AuthorizationReference
  authorization: Record<string, unknown>
  bootstrap: AccountabilityBootstrap
  contract: Record<string, unknown>
  implementationOwner: string
  implementationOwners: string[]
  supervisorId: string | undefined
  contractReviewerId: string | undefined
  implementationReviewerId: string | undefined
  initialBootstrap?: boolean
  initialActorRoles?: Record<string, string>
}

interface InitialActorBootstrapArtifact {
  schemaVersion: 1
...
export function assertAuthorizationContextTime(context: AuthorizationContext, occurredAt: string): void {
...
export interface ActorRegistryEvent {
  schemaVersion: 1
  artifactType: 'engineering-governance-actor-registry-event-v1'
  eventType: 'actor_created' | 'alias_added' | 'actor_revoked' | 'actor_restored'
  sequence: number
  ...
  actor: { id: string; role: string; trustLevel: 'local-claim' }
  occurredAt: string
}
...
function canonicalEventDigest(event: Omit<ActorRegistryEvent, 'eventDigest'>): string {
  return canonicalDigest(event)
}

function safeTaskId(value: string): void {
  if (!/^[a-z0-9][a-z0-9._-]*$/u.test(value)) throw new Error('ACCOUNTABILITY_AUTHORIZATION_TASK_INVALID')
}

function safeAuthorizationPath(projectRoot: string, candidate: string): string {
  const root = realpathSync(resolve(projectRoot))
...
function exactAuthorizationReference(value: unknown): value is AuthorizationReference {
  if (!record(value)) return false
  const keys = Object.keys(value).sort()
  if (JSON.stringify(keys) !== JSON.stringify(['authorizationId', 'path', 'rawSha256', 'semanticDigest'])) return false
...
function authorizationRequirement(contract: Record<string, unknown>, requirementId: string): AuthorizationRequirement {
  const values = Array.isArray(contract.authorizationRequirements) ? contract.authorizationRequirements : []
  const matches = values.filter((value): value is AuthorizationRequirement => (
    record(value)
  ...
  if (matches.length !== 1) throw new Error('ACCOUNTABILITY_AUTHORIZATION_REQUIREMENT_INVALID')
  return matches[0]!
}

function acceptedContractReview(projectRoot: string, taskId: string, contractPath: string, contractRaw: Buffer, contractDigest: string): void {
  const path = safeAuthorizationPath(projectRoot, `.delivery/tasks/${taskId}/contract-review.yaml`)
  const review = parse(readFileSync(path, 'utf8')) as unknown
  if (!record(review) || !validateDocument('contract-review', review).valid) throw new Error('ACCOUNTABILITY_AUTHORIZATION_REVIEW_INVALID')
...
function loadBootstrap(projectRoot: string, taskId: string): AccountabilityBootstrap {
  ...
  return value as unknown as AccountabilityBootstrap
}

function initialBootstrapContext(
  projectRoot: string,
  reference: AuthorizationReference,
  artifact: InitialActorBootstrapArtifact,
  occurredAt: string,
): AuthorizationContext {
  if (artifact.projectRoot !== projectRoot) throw new Error('ACCOUNTABILITY_INITIAL_BOOTSTRAP_PROJECT_INVALID')
  if (!policyDigestAllowedForProject(projectRoot, artifact.policyDigest)) throw new Error('ACCOUNTABILITY_INITIAL_BOOTSTRAP_POLICY_INVALID')
  ...
  const bootstrapActors: AccountabilityBootstrapActor[] = artifact.actors.map((actor) => ({
  ...
  const actorForRole = (role: string): string | undefined => artifact.actors.find((actor) => actor.role === role)?.actorId
  const context: AuthorizationContext = {
    reference,
    authorization: artifact as unknown as Record<string, unknown>,
    bootstrap: {
...
function authorizationExceptionBinds(bootstrap: AccountabilityBootstrap, reference: AuthorizationReference): boolean {
...
export function validateAuthorizationReference(projectRoot: string, value: unknown, occurredAt: string): AuthorizationContext {
  if (!exactAuthorizationReference(value)) throw new Error('ACCOUNTABILITY_AUTHORIZATION_REFERENCE_INVALID')
  const root = realpathSync(resolve(projectRoot))
  const path = safeAuthorizationPath(root, value.path)
  const raw = readFileSync(path)
  if (sha256(raw) !== value.rawSha256) throw new Error('ACCOUNTABILITY_AUTHORIZATION_RAW_MISMATCH')
  let authorization: unknown
  try { authorization = JSON.parse(raw.toString('utf8')) } catch { throw new Error('ACCOUNTABILITY_AUTHORIZATION_JSON_INVALID') }
  if (record(authorization) && authorization.artifactType === 'engineering-governance-initial-actor-bootstrap-v1') {
    ...
    if (path !== expectedPath) throw new Error('ACCOUNTABILITY_INITIAL_BOOTSTRAP_PATH_INVALID')
    if (!validateDocument('initial-actor-bootstrap', authorization).valid) throw new Error('ACCOUNTABILITY_INITIAL_BOOTSTRAP_SCHEMA_INVALID')
    return initialBootstrapContext(root, value, authorization as unknown as InitialActorBootstrapArtifact, occurredAt)
  }
  if (!record(authorization) || !validateDocument('authorization', authorization).valid) throw new Error('ACCOUNTABILITY_AUTHORIZATION_SCHEMA_INVALID')
  ...
  const contract = parse(contractRaw.toString('utf8')) as unknown
  ...
  if (canonicalDigest(unsignedContract) !== contractDigest) throw new Error('ACCOUNTABILITY_AUTHORIZATION_CONTRACT_DIGEST_INVALID')
  ...
  if (!authorizationExceptionBinds(bootstrap, value)) throw new Error('ACCOUNTABILITY_AUTHORIZATION_BOOTSTRAP_BINDING_INVALID')
  const context: AuthorizationContext = {
    reference: value,
    authorization,
    bootstrap,
...
function expectedRole(context: AuthorizationContext, actorId: string): string | undefined {
  const initialRole = context.initialActorRoles?.[actorId]
  if (initialRole !== undefined) return initialRole
  if (context.implementationOwners.includes(actorId)) return 'implementation-owner'
  if (actorId === context.supervisorId) return 'supervisor'
  if (actorId === context.contractReviewerId) return 'contract-reviewer'
  if (actorId === context.implementationReviewerId) return 'implementation-reviewer'
  for (const source of context.bootstrap.sources.filter((candidate) => candidate.kind === 'review')) {
    try {
      ...
      const review = parse(readFileSync(path, 'utf8')) as unknown
      if (record(review) && record(review.reviewer) && normalizeActorId(String(review.reviewer.id)) === actorId) return 'contract-reviewer'
    } catch { /* source identity is independently validated by bootstrap verification */ }
  }
  if (actorId.includes('implementation-reviewer')) return 'implementation-reviewer'
  if (actorId.includes('contract-reviewer')) return 'contract-reviewer'
  return undefined
}

function parseEvent(projectRoot: string, line: string, sequence: number, previous: ActorRegistryEvent | undefined): ActorRegistryEvent {
  let decoded: unknown
  try { decoded = JSON.parse(line) } catch { throw new Error('ACCOUNTABILITY_REGISTRY_JSON_INVALID') }
  if (!record(decoded) || !validateDocument('actor-registry-event', decoded).valid) throw new Error('ACCOUNTABILITY_REGISTRY_SCHEMA_INVALID')
  const parsed = decoded as unknown as ActorRegistryEvent
  if (parsed.sequence !== sequence) throw new Error('ACCOUNTABILITY_REGISTRY_SEQUENCE_INVALID')
  const { eventDigest, ...unsigned } = parsed
  if (canonicalEventDigest(unsigned) !== eventDigest) throw new Error('ACCOUNTABILITY_REGISTRY_DIGEST_INVALID')
  const expectedPrevious = previous?.eventDigest ?? ACCOUNTABILITY_GENESIS_DIGEST
  if (parsed.priorEventDigest !== expectedPrevious) throw new Error('ACCOUNTABILITY_REGISTRY_CHAIN_INVALID')
  if (!policyDigestAllowedForProject(projectRoot, parsed.policyDigest)) throw new Error('ACCOUNTABILITY_REGISTRY_POLICY_INVALID')
  if (previous !== undefined && Date.parse(parsed.occurredAt) < Date.parse(previous.occurredAt)) throw new Error('ACCOUNTABILITY_REGISTRY_TIME_ORDER_INVALID')
  const actorId = normalizeActorId(parsed.actorId)
  const aliases = parsed.aliases.map(normalizeActorId)
  if (normalizeActorId(parsed.actor.id) !== actorId) throw new Error('ACCOUNTABILITY_REGISTRY_ACTOR_BINDING_INVALID')
  if (new Set(aliases).size !== aliases.length || aliases.includes(actorId)) throw new Error('ACCOUNTABILITY_ALIAS_INVALID')
  return { ...parsed, actorId, aliases: [...aliases].sort() }
}

function applyRegistryEvent(
  projectRoot: string,
  event: ActorRegistryEvent,
  actors: Map<string, RegisteredActor>,
  aliases: Map<string, string>,
  priorEvents: ActorRegistryEvent[],
  authorizationContexts: Map<string, AuthorizationContext>,
): void {
  const actorId = event.actorId
  const current = actors.get(actorId)
  if (
    (event.eventType === 'actor_created' && current !== undefined)
  ...
  if (current !== undefined) {
    if (event.actor.role !== current.role || event.actor.trustLevel !== current.trustLevel) throw new Error('ACCOUNTABILITY_REGISTRY_TRANSITION_INVALID')
    ...
    if (event.eventType === 'alias_added') {
      if (before === after || current.aliases.some((alias) => !event.aliases.includes(alias))) throw new Error('ACCOUNTABILITY_REGISTRY_TRANSITION_INVALID')
    } else if (before !== after) throw new Error('ACCOUNTABILITY_REGISTRY_TRANSITION_INVALID')
  }
  ...
  if (bootstrapActor === undefined) throw new Error('ACCOUNTABILITY_REGISTRY_ACTOR_NOT_AUTHORIZED')
  const allowedAliases = bootstrapActor.aliases.map(normalizeActorId).sort()
  if (event.aliases.some((alias) => !allowedAliases.includes(alias))) throw new Error('ACCOUNTABILITY_REGISTRY_ALIAS_NOT_AUTHORIZED')
  const role = expectedRole(context, actorId)
  if (role === undefined || event.actor.role !== role) throw new Error('ACCOUNTABILITY_REGISTRY_ROLE_NOT_AUTHORIZED')
  if (event.eventType !== 'actor_created') {
    ...
    if (reused) throw new Error('ACCOUNTABILITY_AUTHORIZATION_REPLAYED')
  }
  for (const alias of event.aliases) {
    const owner = aliases.get(alias)
    if (owner !== undefined && owner !== actorId) throw new Error('ACCOUNTABILITY_ALIAS_DUPLICATE')
...
export function readActorRegistry(
  projectRoot: string,
  authorizationContexts: Map<string, AuthorizationContext> = new Map(),
): { events: ActorRegistryEvent[]; actors: RegisteredActor[] } {
  assertAccountabilityPolicy(projectRoot)
  const path = registryPath(projectRoot)
  if (!existsSync(path)) return { events: [], actors: [] }
  if (lstatSync(path).isSymbolicLink() || !lstatSync(path).isFile() || realpathSync(path) !== path) throw new Error('ACCOUNTABILITY_REGISTRY_UNSAFE')
  const events: ActorRegistryEvent[] = []
  const actors = new Map<string, RegisteredActor>()
  ...
  for (const context of authorizationContexts.values()) {
    if (!context.initialBootstrap) continue
...
export function resolveRegisteredActor(projectRoot: string, actorOrAlias: string): RegisteredActor {
...
export function appendActorRegistryEvent(
  projectRoot: string,
  event: Omit<ActorRegistryEvent, 'sequence' | 'priorEventDigest' | 'eventDigest' | 'policyDigest' | 'authorization'>,
  authorization: AuthorizationReference,
): ActorRegistryEvent {
  const current = readActorRegistry(projectRoot)
  const unsigned: Omit<ActorRegistryEvent, 'eventDigest'> = {
    ...event,
    authorization,
  ...
  const aliases = new Map<string, string>()
  for (const actor of actors.values()) for (const alias of actor.aliases) aliases.set(alias, actor.actorId)
...
export function assertContainedAuthorityPath(projectRoot: string, candidate: string): string {
  return safeAuthorizationPath(projectRoot, candidate)
}

export function accountabilityGenesisValid(): boolean {
  return sha256(ACCOUNTABILITY_GENESIS_BYTES) === ACCOUNTABILITY_GENESIS_DIGEST
}

export function policyDigestForProject(projectRoot: string): string {
  const policy = parse(readFileSync(join(realpathSync(resolve(projectRoot)), '.delivery', 'policy.yaml'), 'utf8')) as { sopDigest?: unknown }
  if (typeof policy.sopDigest !== 'string' || !/^[a-f0-9]{64}$/u.test(policy.sopDigest)) throw new Error('ACCOUNTABILITY_POLICY_DIGEST_INVALID')
  return policy.sopDigest
}

export function policyDigestsForProject(projectRoot: string): Set<string> {
  const policy = parse(readFileSync(join(realpathSync(resolve(projectRoot)), '.delivery', 'policy.yaml'), 'utf8')) as {
    artifactMapping?: unknown
  }
...
export function policyDigestAllowedForProject(projectRoot: string, digest: string): boolean {
