...
import {
  deriveAccountabilityStatus,
  readAccountabilityEvents,
  type AccountabilityEvent,
  type AccountabilityIncident,
} from '../accountability/derive.js'
import {
...
export interface AccountabilityIncidentPlan {
  schemaVersion: 1
...
function currentDigest(path: string): string | null {
  ...
  if (lstatSync(path).isSymbolicLink() || !lstatSync(path).isFile()) throw new Error(`ACCOUNTABILITY_INCIDENT_PATH_UNSAFE:${path}`)
...
function sourceSemantic(path: string, raw: Buffer): string {
  if (path.endsWith('.jsonl')) {
    try { return canonicalDigest(raw.toString('utf8').split('\n').filter(Boolean).map((line) => JSON.parse(line))) } catch { throw new Error('ACCOUNTABILITY_INCIDENT_EVIDENCE_INVALID') }
  }
  if (['.yaml', '.yml', '.json'].includes(extname(path))) {
    try { return canonicalDigest(parse(raw.toString('utf8'))) } catch { throw new Error('ACCOUNTABILITY_INCIDENT_EVIDENCE_INVALID') }
  }
  return canonicalDigest(raw.toString('utf8'))
}

function evidencePath(projectRoot: string, path: string): string {
  if (isAbsolute(path)) throw new Error('ACCOUNTABILITY_INCIDENT_EVIDENCE_PATH_INVALID')
...
function normalizeIncident(value: unknown, projectRoot: string): AccountabilityIncident {
  const schema = validateDocument('accountability-incident', value)
  if (!schema.valid || !record(value)) throw new Error(`ACCOUNTABILITY_INCIDENT_SCHEMA_INVALID:${schema.errors.join(',')}`)
  const incident = value as unknown as AccountabilityIncident
  if (realpathSync(resolve(incident.projectRoot)) !== projectRoot) throw new Error('ACCOUNTABILITY_INCIDENT_PROJECT_INVALID')
  ...
  if (new Set(evidenceRefs.map((reference) => reference.path)).size !== evidenceRefs.length) throw new Error('ACCOUNTABILITY_INCIDENT_EVIDENCE_DUPLICATED')
...
function planDigest(plan: Omit<AccountabilityIncidentPlan, 'digest'>): string {
...
export function planAccountabilityIncident(projectRootInput: string, inputPathInput: string): AccountabilityIncidentPlan {
  ...
  if (!activeActors.has(incident.subjectActorId)) throw new Error('ACCOUNTABILITY_INCIDENT_SUBJECT_UNAVAILABLE')
  if (incident.reportedBy !== 'user-authority' && !activeActors.has(incident.reportedBy)) throw new Error('ACCOUNTABILITY_INCIDENT_REPORTER_UNAVAILABLE')

  const currentEvents = readAccountabilityEvents(projectRoot)
  if (currentEvents.some((event) => event.incident?.incidentId === incident.incidentId)) throw new Error('ACCOUNTABILITY_INCIDENT_DUPLICATED')
  const previous = currentEvents.at(-1)
  if (previous !== undefined && Date.parse(incident.issuedAt) < Date.parse(previous.occurredAt)) throw new Error('ACCOUNTABILITY_INCIDENT_TIME_ORDER_INVALID')
  const priorOffenses = new Map<string, number>()
  for (const event of currentEvents.filter((event) => event.subjectActorId === incident.subjectActorId && event.eventType === 'finding_assessed')) {
    ...
    if (defectClass !== undefined) priorOffenses.set(defectClass, (priorOffenses.get(defectClass) ?? 0) + 1)
  }
  ...
  const unsigned = {
    ...
    eventType: 'finding_assessed' as const,
  ...
  const event: AccountabilityEvent = { ...unsigned, eventDigest: canonicalDigest(unsigned) }
  const eventSchema = validateDocument('accountability-event', event)
  if (!eventSchema.valid) throw new Error(`ACCOUNTABILITY_INCIDENT_EVENT_SCHEMA_INVALID:${eventSchema.errors.join(',')}`)
  ...
  const writes: PlannedWrite[] = [{ path: eventsPath, beforeDigest: currentDigest(eventsPath), after: `${existing}${JSON.stringify(event)}\n`, mode: 0o644 }]
  const guards = [
    guard(inputPath),
  ...
  const withoutDigest = {
    schemaVersion: 1 as const,
    artifactType: 'engineering-governance-accountability-incident-plan-v1' as const,
    projectRoot,
    inputPath,
    event,
    writes,
...
export function summarizeAccountabilityIncidentPlan(plan: AccountabilityIncidentPlan): object {
  return {
    schemaVersion: plan.schemaVersion,
    artifactType: plan.artifactType,
    projectRoot: plan.projectRoot,
...
export function applyAccountabilityIncident(plan: AccountabilityIncidentPlan, expectedDigest: string): object {
  if (plan.digest !== expectedDigest || planDigest({
    ...
    writes: plan.writes,
    guards: plan.guards,
  }) !== plan.digest) throw new Error('ACCOUNTABILITY_INCIDENT_PLAN_MISMATCH')
  ...
  if (event?.eventDigest !== plan.event.eventDigest) throw new Error('ACCOUNTABILITY_INCIDENT_RESULT_INVALID')
  return {
    digest: plan.digest,
