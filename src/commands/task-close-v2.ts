...
export interface HardenedCloseDecision {
  valid: boolean
  errors: string[]
...
function readExact(reference: ArtifactReference, expectedPath: string, label: string): Buffer {
  ...
  if (path !== expectedPath) throw new Error(`${label}_CANONICAL_PATH_MISMATCH`)
  const raw = readFileSync(path)
  if (sha256(raw) !== reference.sha256) throw new Error(`${label}_ARTIFACT_DIGEST_MISMATCH`)
  return raw
}

function statusArtifactErrors(root: string, reference: ArtifactReference): string[] {
  try {
    const unresolved = resolve(reference.path)
    if (lstatSync(unresolved).isSymbolicLink() || !lstatSync(unresolved).isFile()) {
    ...
    if (relativePath.startsWith('..') || isAbsolute(relativePath)) return ['STATUS_ARTIFACT_OUTSIDE_PROJECT']
...
export function verifyHardenedClose(closurePathInput: string, now = new Date()): HardenedCloseDecision {
  ...
  try {
    ...
    closure = parse(readFileSync(closurePath, 'utf8')) as ClosureV2
  } catch {
  ...
  let review: ReviewV2
  try {
    contract = parse(readExact(closure.contract, join(taskDirectory, 'contract.yaml'), 'CONTRACT').toString('utf8')) as ContractV2
    candidate = parse(readExact(closure.candidate, join(taskDirectory, 'candidate.yaml'), 'CANDIDATE').toString('utf8')) as CandidateV2
    verification = JSON.parse(readExact(
      closure.verification,
      join(taskDirectory, 'verification.json'),
      'VERIFICATION',
    ).toString('utf8')) as VerificationV2
    review = parse(readExact(closure.review, join(taskDirectory, 'review.yaml'), 'REVIEW').toString('utf8')) as ReviewV2
  } catch (error) {
  ...
  if (!contractSchema.valid) errors.push(...contractSchema.errors.map((error) => `CONTRACT_INVALID:${error}`))
    ...
    if (closure.contract.digest !== contractDigest) errors.push('CLOSURE_CONTRACT_DIGEST_MISMATCH')
  ...
  if (canonicalDigest(candidate) !== closure.candidate.digest) errors.push('CLOSURE_CANDIDATE_DIGEST_MISMATCH')
  if (!validateDocument('verification', verification).valid || verification.schemaVersion !== 2) {
    errors.push('VERIFICATION_SCHEMA_INVALID')
  }
  if (!validateDocument('review', review).valid || review.schemaVersion !== 2) errors.push('REVIEW_SCHEMA_INVALID')
  if (review.decision !== 'ACCEPTED' || review.findings.length > 0) errors.push('REVIEW_NOT_ACCEPTED')
  ...
  if (!ledger.valid) errors.push(...ledger.errors.map((error) => `TASK_LEDGER_INVALID:${error}`))
    ...
    if (accepted.eventDigest !== closure.acceptedEventDigest) errors.push('CLOSURE_ACCEPTED_EVENT_MISMATCH')
  ...
  for (const status of closure.statusArtifacts) errors.push(...statusArtifactErrors(projectRoot, status))
  try {
