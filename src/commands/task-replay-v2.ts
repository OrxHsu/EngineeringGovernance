...
import { captureRepositorySet, type CheckoutSnapshot } from '../evidence/checkout-snapshot.js'
...
interface ReplayGatePlan {
  ...
  observerPolicy: FrozenGate['observerPolicy']
}

export interface CandidateReplayPlan {
  schemaVersion: 2
...
interface ReplayExecution {
  acceptanceId: string
  gateDigest: string
  command: ReplayGatePlan['command']
...
export interface ReplayVerificationArtifact {
  schemaVersion: 2
  ...
  verifiedAt: string
  decision: 'eligible' | 'failed'
}

export interface ReplayVerificationResult {
  errors: string[]
  reference?: ArtifactReference & { planDigest: string }
}
...
function normalizeReplaySnapshots(snapshots: CheckoutSnapshot[], taskId: string): CheckoutSnapshot[] {
...
function replayCheckoutSnapshots(repositories: ContractV2['repositories'], taskId: string): CheckoutSnapshot[] {
  return normalizeReplaySnapshots(captureRepositorySet(repositories), taskId)
}

function safeFile(pathInput: string, label: string): { path: string; raw: Buffer } {
...
function loadCandidate(candidatePathInput: string, requireCandidateState = true): {
  ...
  const candidate = parse(candidateArtifact.raw.toString('utf8')) as CandidateV2
  const candidateSchema = validateDocument('candidate', candidate)
  if (!candidateSchema.valid || candidate.schemaVersion !== 2) throw new Error('CANDIDATE_SCHEMA_INVALID')
  ...
  if (relativeTask !== `.delivery/tasks/${candidate.taskId}`) throw new Error('CANDIDATE_TASK_DIRECTORY_MISMATCH')
  ...
  const contract = parse(contractArtifact.raw.toString('utf8')) as ContractV2
  const validation = validateHardenedTaskContract(contract)
  if (!validation.valid) throw new Error(`CONTRACT_INVALID:${validation.errors.join(',')}`)
  if (candidate.taskId !== contract.taskId) throw new Error('CANDIDATE_TASK_ID_MISMATCH')
...
function replayPlanForLoaded(
  loaded: ReturnType<typeof loadCandidate>,
  checkoutSnapshots: CheckoutSnapshot[],
): CandidateReplayPlan {
  const requiredGates = loaded.contract.acceptance.filter((gate) => gate.observerPolicy.replay === 'required')
  if (requiredGates.length === 0) throw new Error('REPLAY_NOT_REQUIRED')
  const gates = requiredGates.map((gate): ReplayGatePlan => {
    const repository = loaded.contract.repositories.find((item) => item.id === gate.command.repositoryId)
    if (repository === undefined) throw new Error(`REPLAY_REPOSITORY_UNKNOWN:${gate.id}`)
    const cwd = realpathSync(resolve(repository.path, gate.command.cwd))
    const relativeCwd = relative(repository.path, cwd)
    if (relativeCwd.startsWith('..') || isAbsolute(relativeCwd)) throw new Error(`REPLAY_CWD_OUTSIDE_REPOSITORY:${gate.id}`)
    return {
  ...
  const unsigned = {
    ...
    artifactType: 'sop-replay-plan-v2' as const,
...
export function planCandidateReplay(candidatePath: string): CandidateReplayPlan {
  const loaded = loadCandidate(candidatePath)
  return replayPlanForLoaded(
    loaded,
    replayCheckoutSnapshots(loaded.contract.repositories, loaded.contract.taskId),
  )
}

function executeGate(plan: CandidateReplayPlan, gate: ReplayGatePlan, repositories: ContractV2['repositories']): ReplayExecution {
  ...
  if (exitCode !== gate.observerPolicy.expectedExitCode) policyErrors.push('REPLAY_EXIT_CODE_MISMATCH')
  ...
  if (gate.observerPolicy.output === 'exact') {
    if (sha256(stdout) !== gate.observerPolicy.expectedStdoutSha256) policyErrors.push('REPLAY_STDOUT_EXACT_MISMATCH')
    if (sha256(stderr) !== gate.observerPolicy.expectedStderrSha256) policyErrors.push('REPLAY_STDERR_EXACT_MISMATCH')
  }
...
export function applyCandidateReplay(
  plan: CandidateReplayPlan,
  approvedDigest: string,
): { path: string; sha256: string; artifact: ReplayVerificationArtifact } {
  ...
  if (canonicalDigest(currentPlan) !== canonicalDigest(plan)) throw new Error('REPLAY_PLAN_DRIFTED')
  ...
  if (existsSync(planPath)) throw new Error('REPLAY_APPROVED_PLAN_ALREADY_EXISTS')
  if (existsSync(outputPath)) throw new Error('REPLAY_VERIFICATION_ALREADY_EXISTS')
  ...
  if (firstSnapshots === undefined || JSON.stringify(
    normalizeReplaySnapshots(firstSnapshots, loaded.contract.taskId),
  ) !== JSON.stringify(plan.checkoutSnapshots)) {
    throw new Error('REPLAY_CHECKOUT_DRIFTED')
  }
  const identity = governanceIdentity()
  const artifact: ReplayVerificationArtifact = {
    schemaVersion: 2,
    artifactType: 'sop-replay-verification-v2',
  ...
  if (!schema.valid) throw new Error(`REPLAY_VERIFICATION_SCHEMA_INVALID:${schema.errors.join(',')}`)
...
function sameIdentities(left: ImplementationIdentity[], right: ImplementationIdentity[]): boolean {
  const canonical = (values: ImplementationIdentity[]) => [...values]
    .sort((a, b) => a.repositoryId.localeCompare(b.repositoryId))
  return JSON.stringify(canonical(left)) === JSON.stringify(canonical(right))
}

function approvedReplayPlan(loaded: ReturnType<typeof loadCandidate>): CandidateReplayPlan {
  const planPath = join(dirname(loaded.candidatePath), 'replay-plan.json')
  if (!existsSync(planPath)) throw new Error('REPLAY_APPROVED_PLAN_REQUIRED')
  let plan: CandidateReplayPlan
  try {
    const file = safeFile(planPath, 'REPLAY_APPROVED_PLAN')
    plan = JSON.parse(file.raw.toString('utf8')) as CandidateReplayPlan
  } catch {
  ...
  const { digest, ...unsigned } = plan
  if (canonicalDigest(unsigned) !== digest) throw new Error('REPLAY_APPROVED_PLAN_INVALID')
  const current = replayPlanForLoaded(loaded, plan.checkoutSnapshots)
  if (canonicalDigest(current) !== canonicalDigest(plan)) throw new Error('REPLAY_APPROVED_PLAN_DRIFTED')
  return plan
}

export function verifyCandidateReplay(
  candidatePath: string,
  ...
  if (!existsSync(outputPath)) {
    const plan = replayPlanForLoaded(
      loaded,
      replayCheckoutSnapshots(loaded.contract.repositories, loaded.contract.taskId),
    )
    return { errors: [`EVIDENCE_REPLAY_APPROVAL_REQUIRED:${plan.digest}`] }
  }
  let artifact: ReplayVerificationArtifact
  let raw: Buffer
  try {
    const file = safeFile(outputPath, 'REPLAY_VERIFICATION')
    raw = file.raw
    artifact = JSON.parse(raw.toString('utf8')) as ReplayVerificationArtifact
  } catch {
    return { errors: ['REPLAY_VERIFICATION_UNREADABLE'] }
  }
  const errors: string[] = []
  const schema = validateDocument('replay-verification', artifact)
  if (!schema.valid) return { errors: schema.errors.map((error) => `REPLAY_VERIFICATION_SCHEMA_INVALID:${error}`) }
  const firstExecution = artifact.executions[0]
  if (firstExecution === undefined) return { errors: ['REPLAY_EXECUTION_SET_MISMATCH'] }
  let plan: CandidateReplayPlan
  try {
  ...
  const executionSnapshots = normalizeReplaySnapshots(
  ...
  if (artifact.planDigest !== plan.digest) errors.push('REPLAY_APPROVED_PLAN_REQUIRED')
  ...
  if (artifact.executions.length !== plan.gates.length) errors.push('REPLAY_EXECUTION_SET_MISMATCH')
  for (const [index, gate] of plan.gates.entries()) {
    ...
    const recomputedPolicyErrors: string[] = []
    ...
    if (gate.observerPolicy.output === 'exact' && (
      sha256(execution.stdout) !== gate.observerPolicy.expectedStdoutSha256
  ...
  if (!Number.isFinite(verifiedAt) || age < 0 || age > maximumAgeMs) errors.push('REPLAY_VERIFICATION_TIME_INVALID')
  if (artifact.decision !== 'eligible') errors.push('REPLAY_VERIFICATION_FAILED')
  return {
